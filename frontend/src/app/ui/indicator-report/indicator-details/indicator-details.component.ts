...
export class IndicatorDetailsComponent extends BaseComponent implements OnInit, OnChanges {
	...
	constructor(
		...
		protected installationConfigurationService: InstallationConfigurationService,
	...
	protected isAvailableForPresentation(item: string): boolean {
		if (!this.installationConfigurationService.availableForPresentation || this.installationConfigurationService.availableForPresentation.length === 0) return true
		return this.installationConfigurationService.availableForPresentation.includes(item);
	}
}
