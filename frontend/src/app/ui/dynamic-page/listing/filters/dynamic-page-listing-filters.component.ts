...
import { DynamicPageFilter } from "@app/core/query/dynamic-page.lookup";
import { BaseComponent } from '@common/base/base.component';
import { MatCheckboxChange } from '@angular/material/checkbox';

@Component({
	selector: "app-dynamic-page-listing-filters",
	templateUrl: "./dynamic-page-listing-filters.component.html",
	styleUrls: ["./dynamic-page-listing-filters.component.scss"],
})
export class DynamicPageListingFiltersComponent
	extends BaseComponent
	implements OnInit, OnChanges
{
	@Input() filter: DynamicPageFilter;
	@Output() filterChange = new EventEmitter<DynamicPageFilter>();
	// panelExpanded = false;
	...
	//
	visibleFields(filter: DynamicPageFilter) {
		this.filterSelections["isActiveOption"] =
			filter.isActive && filter.isActive.length !== 0 ? true : false;
		this.filterSelections["likeOption"] =
			filter.like && filter.like.length !== 0 ? true : false;
	}
	...
	get isActive(): boolean {
		return this.filter.isActive
			? this.filter.isActive.includes(IsActive.Inactive)
			: true;
	}
	...
	countOpenFilters(): number {
		const filtered = Object.entries(this.filterSelections).filter(
			([k, v]) => v === true
		);
	...
	checkBoxChanged(event: MatCheckboxChange, filter: string) {
		if (!event.checked && this.filter[filter] != null) {
			this.filter[filter] =
				filter !== "isActive" ? undefined : [IsActive.Active];
			this.filterChange.emit(this.filter);
		}
	}
}
