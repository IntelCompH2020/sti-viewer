...
export class UserListingComponent extends BaseListingComponent<UserServiceUser, UserLookup> implements OnInit {
	...
	protected initializeLookup(): UserLookup {
		...
		lookup.order = { items: [nameof<UserServiceUser>(x => x.lastName)] };
		lookup.project = {
			fields: [
	...
	protected setupColumns() {
		this.gridColumns.push(...[{
			prop: nameof<UserServiceUser>(x => x.lastName),
			sortable: true,
			languageName: 'USER-SERVICE.USER-LISTING.FIELDS.NAME'
		},
		{
			prop: nameof<UserServiceUser>(x => x.firstName),
			sortable: true,
			languageName: 'USER-SERVICE.USER-LISTING.FIELDS.NAME'
		},
		{
			...
			valueFunction: (x: UserServiceUser) => this.enumUtils.toIsActiveString(x.isActive),
	...
	private getEmail(user: UserServiceUser): string {
		if (!user || !Array.isArray(user.contacts)) { return '-'; }
