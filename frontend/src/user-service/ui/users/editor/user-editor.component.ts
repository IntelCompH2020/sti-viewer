...
export class UserEditorComponent extends BaseEditor<UserEditorModel, UserServiceUser> implements OnInit {
	...
	persistEntity(onSuccess?: (response: any) => void): void { // * DONE
		this.formGroup.value.contacts = this.formGroup.value.contacts.filter(x => x.value);
	...
	buildForm(): void { //* DONE
		...
		this.filteredCultures = this.isNew ?
			this.cultureValues :
			this.cultureValues.filter((culture) => culture.name === this.editorModel.culture);
		this.filteredTimezones = this.isNew ?
			this.timezoneValues :
			this.timezoneValues.filter((zone) => zone === this.editorModel.timezone);
	...
	prepareForm(data: UserServiceUser): void { //* DONE NEEDS REVISITING
		...
		try {
			this.editorModel = data ? new UserEditorModel().fromModel(data) : new UserEditorModel();
