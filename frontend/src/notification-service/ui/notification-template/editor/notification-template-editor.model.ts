...
export class NotificationFieldOptionsEditorModel implements NotificationFieldOptions {
	...
	buildForm(context: ValidationContext = null, baseProperty: string = null, disabled: boolean = false, validationErrorModel: ValidationErrorModel): UntypedFormGroup {
		...
		const mandatoryFormArray = new Array<UntypedFormControl>();
		if (this.mandatory) {
			this.mandatory.forEach((element, index) => {
				mandatoryFormArray.push(new UntypedFormControl({ value: element, disabled: disabled }, [BackendErrorValidator(this.validationErrorModel, this.helperGetValidation(baseProperty, 'Mandatory[' + index + ']'))]));
			});
		}
	...
	buildMandatoryItemForm(value: string, index: number, disabled: boolean = false, baseProperty: string): UntypedFormControl {
		return new UntypedFormControl({ value: value, disabled: disabled }, [BackendErrorValidator(this.validationErrorModel, this.helperGetValidation(baseProperty, `Mandatory[${index}]`))]);
	}

	helperReapplyMandatoryItemsValidators(details: UntypedFormArray) {
	...
	buildOptionalItemForm(item: NotificationFieldInfoEditorModel, index: number, disabled: boolean = false): UntypedFormGroup {
		return item.buildForm(null, `Options[${index}]`, disabled, this.validationErrorModel);
	}

	helperReapplyOptionalItemsValidators(array: UntypedFormArray) {
