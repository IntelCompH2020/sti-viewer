...
export class UserProfileNotifierListEditorModel implements UserNotificationPreferencePersist {
	...
	buildForm(context: ValidationContext = null, disabled: boolean = false, availableNotificationTypes: NotificationType[]): UntypedFormGroup {
		if (context == null) { context = this.createValidationContext(); }

		const notificationPreferencesFormGroup = this.formBuilder.group({});
		availableNotificationTypes.forEach(type => {
			notificationPreferencesFormGroup.addControl(type, new UntypedFormControl(this.notificationPreferences ? this.notificationPreferences[type] : undefined));
		});
