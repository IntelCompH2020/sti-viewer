...
public class InAppNotificationDeleter implements Deleter {
	...
	public void deleteAndSaveByIds(List<UUID> ids) throws InvalidApplicationException {
		...
		List<InAppNotificationEntity> data = this.queryFactory.query(InAppNotificationQuery.class).ids(ids).collect();
		logger.trace("received {} items", Optional.of(data).map(List::size).orElse(0));
		this.deleteAndSave(data);
	}

	public void deleteAndSave(List<InAppNotificationEntity> data) throws InvalidApplicationException {
	...
	public void delete(List<InAppNotificationEntity> datas) throws InvalidApplicationException {
		logger.debug("will delete {}  items", Optional.ofNullable(datas).map(List::size).orElse(0));
		if (datas == null || datas.isEmpty()) return;

		List<UUID> ids = datas.stream().map(InAppNotificationEntity::getId).distinct().collect(Collectors.toList());

		Instant now = Instant.now();

		for (InAppNotificationEntity item : datas) {
