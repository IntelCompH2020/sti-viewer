...
import gr.cite.notification.data.UserInvitationEntity;
...
public class UserInvitationQuery extends QueryBase<UserInvitationEntity> {
	...
	protected String fieldNameOf(FieldResolver item) {
		if (item.match(UserInvitation._id)) return UserInvitationEntity._id;
		else if (item.match(UserInvitation._tenant, Tenant._id)) return UserInvitationEntity._tenantId;
		else if (item.prefix(UserInvitation._tenant)) return UserInvitationEntity._tenantId;
		else if (item.match(UserInvitation._token)) return UserInvitationEntity._token;
		else if (item.match(UserInvitation._email)) return UserInvitationEntity._email;
		else if (item.match(UserInvitation._isConsumed)) return UserInvitationEntity._isConsumed;
		else if (item.match(UserInvitation._expiresAt)) return UserInvitationEntity._expiresAt;
		else if (item.match(UserInvitation._createdAt)) return UserInvitationEntity._createdAt;
		else if (item.match(UserInvitation._updatedAt)) return UserInvitationEntity._updatedAt;
		else if (item.match(UserInvitation._hash)) return UserInvitationEntity._updatedAt;
		else return null;
	}
