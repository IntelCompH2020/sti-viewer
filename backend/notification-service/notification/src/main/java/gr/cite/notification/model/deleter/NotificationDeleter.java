...
public class NotificationDeleter implements Deleter {
	...
	public void deleteAndSaveByIds(List<UUID> ids) throws InvalidApplicationException {
		...
		List<NotificationEntity> data = this.queryFactory.query(NotificationQuery.class).ids(ids).collect();
		logger.trace("received {} items", Optional.of(data).map(List::size).orElse(0));
		this.deleteAndSave(data);
	}

	public void deleteAndSave(List<NotificationEntity> data) throws InvalidApplicationException {
	...
	public void delete(List<NotificationEntity> datas) throws InvalidApplicationException {
		logger.debug("will delete {}  items", Optional.ofNullable(datas).map(List::size).orElse(0));
		if (datas == null || datas.isEmpty()) return;

		List<UUID> ids = datas.stream().map(NotificationEntity::getId).distinct().collect(Collectors.toList());
