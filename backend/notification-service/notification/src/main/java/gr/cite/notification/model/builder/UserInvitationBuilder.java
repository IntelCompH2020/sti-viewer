...
public class UserInvitationBuilder extends BaseBuilder<UserInvitation, UserInvitationEntity> {
	...
	public List<UserInvitation> build(FieldSet fields, List<UserInvitationEntity> data) throws MyApplicationException {
		...
		List<UserInvitation> models = new ArrayList<>();

		for (UserInvitationEntity d : data) {
			UserInvitation m = new UserInvitation();
			if (fields.hasField(this.asIndexer(UserInvitation._token))) m.setToken(d.getToken());
			if (fields.hasField(this.asIndexer(UserInvitation._email))) m.setEmail(d.getEmail());
			if (fields.hasField(this.asIndexer(UserInvitation._isConsumed))) m.setConsumed(d.getConsumed());
			if (fields.hasField(this.asIndexer(UserInvitation._expiresAt))) m.setExpiresAt(d.getExpiresAt());
			if (fields.hasField(this.asIndexer(UserInvitation._createdAt))) m.setCreatedAt(d.getCreatedAt());
			if (fields.hasField(this.asIndexer(UserInvitation._updatedAt))) m.setUpdatedAt(d.getUpdatedAt());
			if (fields.hasField(this.asIndexer(UserInvitation._hash))) m.setHash(this.hashValue(Instant.ofEpochMilli(d.getUpdatedAt().longValue())));
