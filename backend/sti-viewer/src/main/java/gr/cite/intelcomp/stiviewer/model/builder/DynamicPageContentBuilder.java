...
public class DynamicPageContentBuilder extends BaseBuilder<DynamicPageContent, DynamicPageContentEntity> {
    ...
    public List<DynamicPageContent> build(FieldSet fields, List<DynamicPageContentEntity> data) throws MyApplicationException {
        ...
        List<DynamicPageContent> models = new ArrayList<>(100);

        for (DynamicPageContentEntity d : data) {
            DynamicPageContent m = new DynamicPageContent();
            if (fields.hasField(this.asIndexer(DynamicPageContent._id)))
                m.setId(d.getId());
            if (fields.hasField(this.asIndexer(DynamicPageContent._isActive)))
                m.setIsActive(d.getIsActive());
            if (fields.hasField(this.asIndexer(DynamicPageContent._language)))
                m.setLanguage(d.getLanguage());
            if (fields.hasField(this.asIndexer(DynamicPageContent._title)))
                m.setTitle(d.getTitle());
            if (fields.hasField(this.asIndexer(DynamicPageContent._content)))
                m.setContent(d.getContent());
            if (fields.hasField(this.asIndexer(DynamicPageContent._createdAt)))
                m.setCreatedAt(d.getCreatedAt());
            if (fields.hasField(this.asIndexer(DynamicPageContent._updatedAt)))
                m.setUpdatedAt(d.getUpdatedAt());
            if (fields.hasField(this.asIndexer(DynamicPageContent._hash)))
                m.setHash(this.hashValue(d.getUpdatedAt()));
            if (!dynamicPageFields.isEmpty() && dynamicPageItemsMap != null && dynamicPageItemsMap.containsKey(d.getPageId()))
                m.setPage(dynamicPageItemsMap.get(d.getPageId()));
            models.add(m);
        }
    ...
    private Map<UUID, DynamicPage> collectPages(FieldSet fields, List<DynamicPageContentEntity> data) throws MyApplicationException {
        ...
        if (!fields.hasOtherField(this.asIndexer(DynamicPage._id))) {
            itemMap = this.asEmpty(
                    data.stream().map(DynamicPageContentEntity::getPageId).distinct().collect(Collectors.toList()),
                    x -> {
            ...
            DynamicPageQuery q = this.queryFactory.query(DynamicPageQuery.class).authorize(this.authorize).ids(data.stream().map(DynamicPageContentEntity::getPageId).distinct().collect(Collectors.toList()));
