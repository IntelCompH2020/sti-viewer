...
public class ExternalTokenQuery extends QueryBase<ExternalTokenEntity> {
	...
	protected <X, Y> Predicate applyAuthZ(QueryContext<X, Y> queryContext) {
		...
		if (this.authorize.contains(AuthorizationFlags.Permission) && this.authService.authorize(Permission.BrowseExternalToken)) return null;
		UUID ownerId = null;
	...
	protected String fieldNameOf(FieldResolver item) {
		if (item.match(ExternalToken._id)) return ExternalTokenEntity._id;
		else if (item.match(ExternalToken._expiresAt)) return ExternalTokenEntity._expiresAt;
		else if (item.match(ExternalToken._name)) return ExternalTokenEntity._name;
		else if (item.prefix(ExternalToken._tenant, Tenant._id)) return ExternalTokenEntity._tenantId;
		else if (item.prefix(ExternalToken._tenant)) return ExternalTokenEntity._tenantId;
		else if (item.prefix(ExternalToken._owner, User._id)) return ExternalTokenEntity._ownerId;
		else if (item.prefix(ExternalToken._owner)) return ExternalTokenEntity._ownerId;
		else if (item.match(ExternalToken._isActive)) return ExternalTokenEntity._isActive;
		else if (item.match(ExternalToken._type)) return ExternalTokenEntity._type;
		else if (item.match(ExternalToken._createdAt)) return ExternalTokenEntity._createdAt;
		else if (item.match(ExternalToken._updatedAt)) return ExternalTokenEntity._updatedAt;
		else if (item.match(ExternalToken._hash)) return ExternalTokenEntity._updatedAt;
		else return null;
	}
}
