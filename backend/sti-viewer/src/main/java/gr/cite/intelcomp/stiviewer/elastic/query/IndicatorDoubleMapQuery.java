...
public class IndicatorDoubleMapQuery extends ElasticNestedQuery<IndicatorDoubleMapQuery, IndicatorDoubleMapEntity, String> {
	...
	public IndicatorDoubleMapEntity convert(Map<String, Object> rawData, Set<String> columns) {
		...
		if (columns.contains(IndicatorDoubleMapEntity.Fields.key)) mocDoc.setKey(FieldBasedMapper.shallowSafeConversion(rawData.get(IndicatorDoubleMapEntity.Fields.key), String.class));
		if (columns.contains(IndicatorDoubleMapEntity.Fields.val)) mocDoc.setVal(FieldBasedMapper.shallowSafeConversion(rawData.get(IndicatorDoubleMapEntity.Fields.val), Double.class));
		return mocDoc;
	}

	@Override
	protected ElasticField fieldNameOf(FieldResolver item) {
		if (item.match(IndicatorDoubleMapEntity.Fields.key)) return this.elasticFieldOf(IndicatorDoubleMapEntity.Fields.key);
		else if (item.match(IndicatorDoubleMapEntity.Fields.val)) return this.elasticFieldOf(IndicatorDoubleMapEntity.Fields.val);
		else return null;
	}
	...
	protected Boolean supportsMetricAggregate(MetricAggregateType metricAggregateType, FieldResolver item) {
		if (item.match(IndicatorDoubleMapEntity.Fields.val)) return true;
		return false;
	}
