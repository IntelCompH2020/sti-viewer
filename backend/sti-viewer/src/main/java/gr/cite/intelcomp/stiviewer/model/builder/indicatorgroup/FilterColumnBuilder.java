...
public class FilterColumnBuilder extends BaseBuilder<FilterColumn, FilterColumnEntity> {
    ...
    public FilterColumnBuilder(ConventionService conventionService,
                               ConventionService conventionService1) {
    ...
    public List<FilterColumn> build(FieldSet fields, List<FilterColumnEntity> data) throws MyApplicationException {
        ...
        List<FilterColumn> models = new ArrayList<>(100);

        for (FilterColumnEntity d : data) {
            FilterColumn m = new FilterColumn();
            if (fields.hasField(this.asIndexer(FilterColumn._code)) && !conventionService.isNullOrEmpty(d.getCode()))
                m.setCode(d.getCode());
            if (fields.hasField(this.asIndexer(FilterColumn._dependsOnCode)) && !conventionService.isNullOrEmpty(d.getDependsOnCode()))
                m.setDependsOnCode(d.getDependsOnCode());
            models.add(m);
        }
