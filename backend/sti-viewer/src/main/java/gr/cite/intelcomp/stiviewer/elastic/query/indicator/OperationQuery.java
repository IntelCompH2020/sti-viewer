...
//		}
    ...
    public OperationEntity convert(Map<String, Object> rawData, Set<String> columns) {
        ...
        if (columns.contains(OperationEntity.Fields.op))
            mocDoc.setOp(FieldBasedMapper.shallowSafeConversion(rawData.get(OperationEntity.Fields.op), String.class));
        return mocDoc;
    }

    @Override
    protected ElasticField fieldNameOf(FieldResolver item) {
        if (item.match(Operator._op))
            return this.elasticFieldOf(OperationEntity.Fields.op).disableInfer(true);
        else
            return null;
    }
