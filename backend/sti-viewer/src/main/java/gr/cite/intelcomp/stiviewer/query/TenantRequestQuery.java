...
public class TenantRequestQuery extends QueryBase<TenantRequestEntity> {
    ...
    protected <X, Y> Predicate applyAuthZ(QueryContext<X, Y> queryContext) {
        if (this.authorize.contains(AuthorizationFlags.None))
            return null;
        if (this.authorize.contains(AuthorizationFlags.Permission) && this.authService.authorize(Permission.BrowseTenantRequest))
            return null;
        UUID ownerId = null;
    ...
    protected String fieldNameOf(FieldResolver item) {
        if (item.match(TenantRequestEntity._id))
            return TenantRequestEntity._id;
        else if (item.match(TenantRequest._message))
            return TenantRequestEntity._message;
        else if (item.match(TenantRequest._status))
            return TenantRequestEntity._status;
        else if (item.match(TenantRequest._email))
            return TenantRequestEntity._email;
        else if (item.match(TenantRequest._createdAt))
            return TenantRequestEntity._createdAt;
        else if (item.match(TenantRequest._updatedAt))
            return TenantRequestEntity._updatedAt;
        else if (item.prefix(TenantRequest._forUser))
            return TenantRequestEntity._forUserId;
        else if (item.prefix(TenantRequest._assignedTenant))
            return TenantRequestEntity._assignedTenantId;
        else
            return null;
    }
