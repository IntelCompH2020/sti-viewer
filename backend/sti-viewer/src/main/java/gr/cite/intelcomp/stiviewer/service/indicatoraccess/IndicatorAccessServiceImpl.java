...
public class IndicatorAccessServiceImpl implements IndicatorAccessService {
	...
	private IndicatorAccessConfigEntity buildIndicatorAccessConfig(IndicatorAccessEntity data, FilterColumnConfigPersist persist) {
		IndicatorAccessConfigEntity config = this.jsonHandlingService.fromJsonSafe(IndicatorAccessConfigEntity.class, data.getConfig());
		if (persist == null || this.conventionService.isNullOrEmpty(persist.getColumn()) || conventionService.isListNullOrEmpty(persist.getValues())) throw new MyApplicationException("Required at least one column");
		...
		if (conventionService.isListNullOrEmpty(persist.getValues())) throw new MyApplicationException("Required at least one column");
		
		if (config == null) config = new IndicatorAccessConfigEntity();
		if (config.getGlobalFilterColumns() == null) config.setGlobalFilterColumns(new ArrayList<>());
		...
		for (FilterColumnConfigEntity columnConfigEntity : config.getGlobalFilterColumns()) {
			...
			if (columnConfig.getColumn().equals(persist.getColumn())){
				if (columnConfig.getValues() == null) columnConfig.setValues(new ArrayList<>());
