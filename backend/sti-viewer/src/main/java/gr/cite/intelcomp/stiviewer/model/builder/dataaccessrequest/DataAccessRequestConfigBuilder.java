...
public class DataAccessRequestConfigBuilder extends BaseBuilder<DataAccessRequestConfig, DataAccessRequestConfigEntity> {
    ...
    public List<gr.cite.intelcomp.stiviewer.model.dataaccessrequest.DataAccessRequestConfig> build(FieldSet fields, List<DataAccessRequestConfigEntity> data) throws MyApplicationException {
        ...
        List<gr.cite.intelcomp.stiviewer.model.dataaccessrequest.DataAccessRequestConfig> models = new ArrayList<>(100);
        ...
        for (DataAccessRequestConfigEntity d : data) {
            ...
            if (!indicatorFields.isEmpty() && d.getIndicators() != null)
                m.setIndicators(this.builderFactory.builder(DataAccessRequestIndicatorConfigBuilder.class).authorize(this.authorize).build(indicatorFields, d.getIndicators()));
            if (!indicatorFieldGroups.isEmpty() && d.getIndicatorGroups() != null)
                m.setIndicatorGroups(this.builderFactory.builder(DataAccessRequestIndicatorGroupConfigBuilder.class).authorize(this.authorize).build(indicatorFieldGroups, d.getIndicatorGroups()));
            models.add(m);
        }
