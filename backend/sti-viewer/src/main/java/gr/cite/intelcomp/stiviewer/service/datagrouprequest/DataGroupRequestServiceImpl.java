...
public class DataGroupRequestServiceImpl implements DataGroupRequestService {
    ...
    private static final LoggerService logger = new LoggerService(LoggerFactory.getLogger(DataGroupRequestServiceImpl.class));
    ...
    public DataGroupRequest persist(DataGroupRequestPersist model, FieldSet fields) throws MyForbiddenException, MyValidationException, MyApplicationException, MyNotFoundException, InvalidApplicationException, NoSuchAlgorithmException {
        ...
        if (isUpdate) {
            ...
            if (data == null)
                throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{model.getId(), DataGroupRequest.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private DataGroupRequestConfigEntity mapToDataGroupRequestConfig(DataGroupRequestConfigPersist config) {
        ...
        if (config.getGroupColumns() != null) {
            List<DataGroupColumnEntity> dataGroupColumnEntities = new ArrayList<>(100);
    ...
    private boolean buildGroupInternal(DataGroupRequestEntity request) throws InvalidApplicationException, IOException {
        ...
        if (indicatorGroupEntity == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{config.getIndicatorGroupId(), IndicatorGroupEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private DataGroupInfoEntity buildDataGroupInfoEntity(DataGroupRequestConfigEntity config) {
        ...
        List<DataGroupInfoColumnEntity> dataGroupInfoColumnEntities = new ArrayList<>(100);
    ...
    private IndicatorPointQuery buildIndicatorPointQueryToBuildDataGroup(UUID indicatorId, DataGroupRequestConfigEntity config, IndicatorConfigItem indicatorConfigItem) {
        ...
        List<IndicatorPointKeywordFilter> indicatorPointKeywordFilters = new ArrayList<>(100);
        for (DataGroupColumnEntity groupColumn : config.getGroupColumns()) {
            FieldEntity fieldEntity = indicatorConfigItem.getExtraProps().get(groupColumn.getFieldCode());
            if (fieldEntity == null)
                throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{groupColumn.getFieldCode(), FieldEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
            if (fieldEntity.getBaseType() != IndicatorFieldBaseType.Keyword)
                throw new MyApplicationException("Not supported " + fieldEntity.getBaseType() + "for grouping");
    ...
    private HashMap<String, List<IndicatorPointEntity>> collectItemsToBuildDataGroup(UUID indicatorId, DataGroupRequestConfigEntity config, IndicatorConfigItem indicatorConfigItem, List<String> columnsForDataGroupItemKey) {
        ...
        do {
            if (indicatorPointEntityScrollResponse == null)
                break;

            for (IndicatorPointEntity indicatorPointEntity : indicatorPointEntityScrollResponse.getItems()) {
                StringBuilder indicatorPointEntityKey = new StringBuilder();
                for (String keyColumn : columnsForDataGroupItemKey) {
                    if (indicatorPointEntity.getProperties() != null) {
                        ...
                        if (value != null) indicatorPointEntityKey.append(value);
                    }
                }
                String indicatorPointEntityKeyHash;
                try {
                    indicatorPointEntityKeyHash = this.cipherService.toSha1(indicatorPointEntityKey.toString());
                } catch (NoSuchAlgorithmException e) {
                    indicatorPointEntityKeyHash = indicatorPointEntityKey.toString();
                }
    ...
    private IndicatorPointEntity createDataGroupItemForIndicatorPointEntities(DataGroupRequestEntity request, UUID indicatorId, IndicatorConfigItem indicatorConfigItem, DataGroupInfoEntity dataGroupInfoEntity, List<String> columnsForDataGroupItemKey, List<IndicatorPointEntity> itemsToGroup) {
        ...
        for (Map.Entry<String, FieldEntity> prop : indicatorConfigItem.getExtraProps().entrySet()) {
            indicatorConfigItem.getExtraProps().get(prop.getKey());
                    ...
                    if (propValue != null) {
                        switch (prop.getValue().getBaseType()) {
                            ...
                            case DoubleMap:
                                ...
                                if (finalValDoubleMap == null)
                                    finalValDoubleMap = new ArrayList<>(100);
    ...
    private String computeGroupHash(DataGroupRequestConfigEntity config) throws NoSuchAlgorithmException {
        ...
        if (config.getGroupColumns() != null) {
            ...
            groupColumns.sort(Comparator.comparing(DataGroupColumnEntity::getFieldCode));

            for (DataGroupColumnEntity groupColumn : groupColumns) {
                hashText.append(groupColumn.getFieldCode());

                String[] values = groupColumn.getValues().stream().distinct().sorted().toArray(String[]::new);
