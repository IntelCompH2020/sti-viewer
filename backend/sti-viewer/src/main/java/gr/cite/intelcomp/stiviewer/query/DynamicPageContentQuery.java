...
public class DynamicPageContentQuery extends QueryBase<DynamicPageContentEntity> {
	...
	protected String fieldNameOf(FieldResolver item) {
		if (item.match(DynamicPageContent._id)) return DynamicPageContentEntity._id;
		else if (item.prefix(DynamicPageContent._page)) return DynamicPageContentEntity._pageId;
		else if (item.match(DynamicPageContent._page)) return DynamicPageContentEntity._pageId;
		else if (item.match(DynamicPageContent._title)) return DynamicPageContentEntity._title;
		else if (item.match(DynamicPageContent._content)) return DynamicPageContentEntity._content;
		else if (item.match(DynamicPageContent._language)) return DynamicPageContentEntity._language;
		else if (item.match(DynamicPageContent._createdAt)) return DynamicPageContentEntity._createdAt;
		else if (item.match(DynamicPageContent._updatedAt)) return DynamicPageContentEntity._updatedAt;
		else if (item.match(DynamicPageContent._hash)) return DynamicPageContent._updatedAt;
		else if (item.match(DynamicPageContent._isActive)) return DynamicPageContentEntity._isActive;
		else return null;
	}
