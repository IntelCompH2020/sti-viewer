...
public class DataGroupRequestQuery extends QueryBase<DataGroupRequestEntity> {
	...
	protected <X, Y> Predicate applyAuthZ(QueryContext<X, Y> queryContext) {
		...
		if (this.authorize.contains(AuthorizationFlags.Permission) && this.authService.authorize(Permission.BrowseDataGroupRequest)) return null;
		UUID ownerId = null;
	...
	protected String fieldNameOf(FieldResolver item) {
		if (item.match(DataGroupRequest._id)) return DataGroupRequestEntity._id;
		else if (item.match(DataGroupRequest._status)) return DataGroupRequestEntity._status;
		else if (item.match(DataGroupRequest._isActive)) return DataGroupRequestEntity._isActive;
		else if (item.match(DataGroupRequest._user, UserEntity._id)) return DataGroupRequestEntity._userId;
		else if (item.prefix(DataGroupRequest._user)) return DataGroupRequestEntity._userId;
		else if (item.prefix(DataGroupRequest._tenant, TenantEntity._id)) return DataGroupRequestEntity._tenantId;
		else if (item.prefix(DataGroupRequest._tenant)) return DataGroupRequestEntity._tenantId;
		else if (item.match(DataGroupRequest._createdAt)) return DataGroupRequestEntity._createdAt;
		else if (item.match(DataGroupRequest._updatedAt)) return DataGroupRequestEntity._updatedAt;
		else if (item.match(DataGroupRequest._config)) return DataGroupRequestEntity._config;
		else if (item.prefix(DataGroupRequest._config)) return DataGroupRequestEntity._config;
		else if (item.match(DataGroupRequest._groupHash)) return DataGroupRequestEntity._groupHash;
		else if (item.match(DataGroupRequest._name)) return DataGroupRequestEntity._name;
		else if (item.match(DataGroupRequest._hash)) return DatasetEntity._updatedAt;
		else return null;
	}
