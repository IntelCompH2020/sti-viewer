...
public class IndicatorBuilder extends BaseBuilder<Indicator, IndicatorEntity> {
	...
	@Override
	public List<Indicator> build(FieldSet fields, List<IndicatorEntity> datas) throws MyApplicationException {
		...
		List<Indicator> models = new ArrayList<>();

		FieldSet indicatorAccessesFields = fields.extractPrefixed(this.asPrefix(Indicator._indicatorAccesses));
		Map<UUID, List<IndicatorAccess>> indicatorAccessesMap = this.collectIndicatorAccesses(indicatorAccessesFields, datas);
		// TODO make it in bulk
		FieldSet accessRequestConfigFields = fields.extractPrefixed(this.asPrefix(Indicator._config));

		for (IndicatorEntity d : datas) {
			Indicator m = new Indicator();
			if (fields.hasField(this.asIndexer(Indicator._id))) m.setId(d.getId());
			if (fields.hasField(this.asIndexer(Indicator._code))) m.setCode(d.getCode());
			...
			if (fields.hasField(this.asIndexer(Indicator._isActive))) m.setIsActive(d.getIsActive());
			if (fields.hasField(this.asIndexer(Indicator._hash))) m.setHash(this.hashValue(d.getUpdatedAt()));
			if (!indicatorAccessesFields.isEmpty() && indicatorAccessesMap != null && indicatorAccessesMap.containsKey(d.getId())) m.setIndicatorAccesses(indicatorAccessesMap.get(d.getId()));
			models.add(m);
		}
	...
	private Map<UUID, List<IndicatorAccess>> collectIndicatorAccesses(FieldSet fields, List<IndicatorEntity> datas) throws MyApplicationException {
		if (fields.isEmpty() || datas.isEmpty()) return null;
		this.logger.debug("checking related - {}", IndicatorAccess.class.getSimpleName());

		Map<UUID, List<IndicatorAccess>> itemMap = null;
		FieldSet clone = new BaseFieldSet(fields.getFields()).ensure(this.asIndexer(IndicatorAccess._indicator, Indicator._id));
		IndicatorAccessQuery query = this.queryFactory.query(IndicatorAccessQuery.class).authorize(this.authorize).indicatorIds(datas.stream().map(x -> x.getId()).distinct().collect(Collectors.toList()));
		...
		if (!fields.hasField(this.asIndexer(IndicatorAccess._indicator, Indicator._id))) {
			itemMap.values().stream().flatMap(List::stream).filter(x -> x != null && x.getIndicator() != null).map(x -> {
				x.getIndicator().setId(null);
				return x;
