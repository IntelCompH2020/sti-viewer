...
public class DataGroupRequestConfigBuilder extends BaseBuilder<DataGroupRequestConfig, DataGroupRequestConfigEntity> {
	...
	public List<DataGroupRequestConfig> build(FieldSet fields, List<DataGroupRequestConfigEntity> data) throws MyApplicationException {
		...
		List<DataGroupRequestConfig> models = new ArrayList<>();
		...
		for (DataGroupRequestConfigEntity d : data) {
			...
			if (!groupColumnFields.isEmpty() && d.getGroupColumns() != null) m.setGroupColumns(this.builderFactory.builder(DataGroupColumnBuilder.class).authorize(this.authorize).build(groupColumnFields, d.getGroupColumns()));
			if (!indicatorGroupFields.isEmpty() && indicatorGroupEntitiesMap != null && indicatorGroupEntitiesMap.containsKey(d.getIndicatorGroupId())) m.setIndicatorGroup(this.builderFactory.builder(IndicatorGroupBuilder.class).authorize(this.authorize).build(indicatorGroupFields, indicatorGroupEntitiesMap.get(d.getIndicatorGroupId())));
			models.add(m);
		}
