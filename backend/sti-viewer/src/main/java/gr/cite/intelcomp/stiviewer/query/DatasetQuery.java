...
public class DatasetQuery extends QueryBase<DatasetEntity> {
    ...
    protected String fieldNameOf(FieldResolver item) {
        if (item.match(Dataset._id))
            return DatasetEntity._id;
        else if (item.match(Dataset._title))
            return DatasetEntity._title;
        else if (item.match(Dataset._notes))
            return DatasetEntity._notes;
        else if (item.match(Dataset._isActive))
            return DatasetEntity._isActive;
        else if (item.match(Dataset._createdAt))
            return DatasetEntity._createdAt;
        else if (item.match(Dataset._updatedAt))
            return DatasetEntity._updatedAt;
        else if (item.match(Dataset._hash))
            return DatasetEntity._updatedAt;
        else
            return null;
    }
