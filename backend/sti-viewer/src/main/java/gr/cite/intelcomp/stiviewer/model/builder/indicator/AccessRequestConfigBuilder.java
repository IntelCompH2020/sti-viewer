...
public class AccessRequestConfigBuilder extends BaseBuilder<AccessRequestConfig, AccessRequestConfigEntity> {
    ...
    public List<AccessRequestConfig> build(FieldSet fields, List<AccessRequestConfigEntity> data) throws MyApplicationException {
        ...
        List<AccessRequestConfig> models = new ArrayList<>(100);
        ...
        for (AccessRequestConfigEntity d : data) {
            ...
            if (!filterColumnsFields.isEmpty() && d.getFilterColumns() != null)
                m.setFilterColumns(this.builderFactory.builder(FilterColumnConfigBuilder.class).authorize(this.authorize).build(filterColumnsFields, d.getFilterColumns()));
            models.add(m);
        }
