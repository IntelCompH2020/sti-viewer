...
public class IndicatorAccessQuery extends QueryBase<IndicatorAccessEntity> {
    ...
    protected <X, Y> Predicate applyAuthZ(QueryContext<X, Y> queryContext) {
        if (this.authorize.contains(AuthorizationFlags.None))
            return null;
        if (this.authorize.contains(AuthorizationFlags.Permission) && this.authService.authorize(Permission.BrowseIndicatorAccess))
            return null;
        UUID ownerId = null;
    ...
    protected <X, Y> Predicate applyFilters(QueryContext<X, Y> queryContext) {
        ...
        if (this.hasUser != null) {
            ...
            predicates.add(this.hasUser ? isNull.not() : isNull);
        }
    ...
    protected String fieldNameOf(FieldResolver item) {
        if (item.match(IndicatorAccess._id))
            return IndicatorAccessEntity._id;
        else if (item.match(IndicatorAccess._isActive))
            return IndicatorAccessEntity._isActive;
        else if (item.prefix(IndicatorAccess._user))
            return IndicatorAccessEntity._userId;
        else if (item.prefix(IndicatorAccess._indicator))
            return IndicatorAccessEntity._indicatorId;
        else if (item.match(IndicatorAccess._indicator))
            return IndicatorAccessEntity._indicatorId;
        else if (item.prefix(IndicatorAccess._tenant))
            return IndicatorAccessEntity._tenantId;
        else if (item.match(IndicatorAccess._createdAt))
            return IndicatorAccessEntity._createdAt;
        else if (item.match(IndicatorAccess._updatedAt))
            return IndicatorAccessEntity._updatedAt;
        else if (item.match(IndicatorAccess._config))
            return IndicatorAccessEntity._config;
        else if (item.prefix(IndicatorAccess._config))
            return IndicatorAccessEntity._config;
        else
            return null;
    }
