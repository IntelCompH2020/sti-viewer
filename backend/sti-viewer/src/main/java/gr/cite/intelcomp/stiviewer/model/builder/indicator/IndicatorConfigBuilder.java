...
public class IndicatorConfigBuilder extends BaseBuilder<IndicatorConfig, IndicatorConfigEntity> {
	...
	public List<IndicatorConfig> build(FieldSet fields, List<IndicatorConfigEntity> data) throws MyApplicationException {
		...
		List<IndicatorConfig> models = new ArrayList<>();
		...
		for (IndicatorConfigEntity d : data) {
			...
			if (!filterColumnsFields.isEmpty() && d.getAccessRequestConfig() != null) m.setAccessRequestConfig(this.builderFactory.builder(AccessRequestConfigBuilder.class).authorize(this.authorize).build(filterColumnsFields, d.getAccessRequestConfig()));
			models.add(m);
		}
