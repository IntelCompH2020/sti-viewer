...
public class PortofolioColumnConfigBuilder extends BaseBuilder<PortofolioColumnConfig, PortofolioColumnConfigEntity> {
    ...
    public PortofolioColumnConfigBuilder(BuilderFactory builderFactory,
    ...
    public List<PortofolioColumnConfig> build(FieldSet fields, List<PortofolioColumnConfigEntity> data) throws MyApplicationException {
        ...
        List<PortofolioColumnConfig> IndicatorReportLevelConfigs = new ArrayList<>(100);
        ...
        for (PortofolioColumnConfigEntity d : data) {
            ...
            if (fields.hasField(this.asIndexer(PortofolioColumnConfig._order)))
                m.setOrder(d.getOrder());
            if (fields.hasField(this.asIndexer(PortofolioColumnConfig._major)))
                m.setMajor(d.getMajor());
            if (!filedFields.isEmpty())
                m.setField(this.builderFactory.builder(DataFieldBuilder.class).authorize(this.authorize).build(filedFields, d.getField()));
            if (!dashboardOverrideFields.isEmpty() && d.getDashboardOverrides() != null)
                m.setDashboardOverrides(this.builderFactory.builder(PortofolioColumnDashboardOverrideBuilder.class).authorize(this.authorize).build(dashboardOverrideFields, d.getDashboardOverrides()));
