...
//	}
    ...
    protected String[] getIndex() {
        if (this.indicatorIds != null && !this.indicatorIds.isEmpty()) {
            try {
                List<String> indexNames = new ArrayList<>(100);
                for (UUID indicatorId : this.indicatorIds)
                    indexNames.add(this.elasticIndicatorService.getIndexName(indicatorId));
    ...
    protected QueryBuilder applyAuthZ() {
        if (this.indicatorPointQueryDefinitionEntity == null) {
            ...
            if (this.authorize.contains(AuthorizationFlags.Permission) && this.authService.authorize(Permission.BrowseIndicatorPoint))
                return null;
        }
    ...
    private List<QueryBuilder> buildHierarchyIndicatorColumnAccessQuery(List<HierarchyIndicatorColumnAccess> indicatorColumnAccesses, List<QueryBuilder> orItems, Map<String, String> andItems) {
        ...
        for (String code : group.keySet()) {
            ...
            List<String> lastChildValues = new ArrayList<>(100);
    ...
    protected QueryBuilder applyFilters() {
        ...
        if (fieldLikeFilter != null && fieldLikeFilter.getLike() != null && !fieldLikeFilter.getLike().isBlank() && fieldLikeFilter.getFields() != null) {
            ...
            if (!fieldLikeFilter.getLike().startsWith("*"))
                like = "*" + like;
            if (!fieldLikeFilter.getLike().endsWith("*"))
                like = like + "*";
        ...
        if (dateRangeFilters != null) {
            for (IndicatorPointDateRangeFilter filer : dateRangeFilters) {
                ...
                if (fieldEntity == null || fieldEntity.getBaseType() != IndicatorFieldBaseType.Date)
                    throw new MyApplicationException("invalid field " + filer.getField());
        ...
        if (integerRangeFilters != null) {
            for (IndicatorPointIntegerRangeFilter filer : integerRangeFilters) {
                FieldEntity fieldEntity = this.getOrDefaultFieldEntity(filer.getField(), null);
                if (fieldEntity == null || fieldEntity.getBaseType() != IndicatorFieldBaseType.Integer)
                    throw new MyApplicationException("invalid field " + filer.getField());
                predicates.add(this.rangeQuery(this.fieldNameOf(new FieldResolver(filer.getField())), filer.getFrom(), filer.getTo()));
            }
        }
        ...
        if (doubleRangeFilters != null) {
            for (IndicatorPointDoubleRangeFilter filer : doubleRangeFilters) {
                FieldEntity fieldEntity = this.getOrDefaultFieldEntity(filer.getField(), null);
                if (fieldEntity == null || fieldEntity.getBaseType() != IndicatorFieldBaseType.Double)
                    throw new MyApplicationException("invalid field " + filer.getField());
                predicates.add(this.rangeQuery(this.fieldNameOf(new FieldResolver(filer.getField())), filer.getFrom(), filer.getTo()));
            }
        }
    ...
    public IndicatorPointEntity convert(Map<String, Object> rawData, Set<String> columns) {
        ...
        if (columns.contains(IndicatorPointEntity.Fields.id))
            mocDoc.setId(FieldBasedMapper.shallowSafeConversion(rawData.get(IndicatorPointEntity.Fields.id), UUID.class));
        if (columns.contains(IndicatorPointEntity.Fields.batchId))
            mocDoc.setBatchId(FieldBasedMapper.shallowSafeConversion(rawData.get(IndicatorPointEntity.Fields.batchId), String.class));
        if (columns.contains(IndicatorPointEntity.Fields.timestamp))
            mocDoc.setBatchTimestamp(FieldBasedMapper.shallowSafeConversion(rawData.get(IndicatorPointEntity.Fields.timestamp), Date.class));
        if (columns.contains(IndicatorPointEntity.Fields.batchTimestamp))
            mocDoc.setBatchTimestamp(FieldBasedMapper.shallowSafeConversion(rawData.get(IndicatorPointEntity.Fields.batchTimestamp), Date.class));
        if (columns.contains(IndicatorPointEntity.Fields.groupHash))
            mocDoc.setGroupHash(FieldBasedMapper.shallowSafeConversion(rawData.get(IndicatorPointEntity.Fields.groupHash), String.class));
    ...
    protected ElasticField fieldNameOf(FieldResolver item) {
        if (item.match(IndicatorPoint._id))
            return this.elasticFieldOf(IndicatorPointEntity.Fields.id);
        else if (item.match(IndicatorPoint._score))
            return this.elasticFieldOf(IndicatorPointEntity.Fields.score).disableInfer(true);
        else if (item.match(IndicatorPoint._batchId))
            return this.elasticFieldOf(IndicatorPointEntity.Fields.batchId);
        else if (item.match(IndicatorPoint._batchTimestamp))
            return this.elasticFieldOf(IndicatorPointEntity.Fields.batchTimestamp);
        else if (item.match(IndicatorPoint._timestamp))
            return this.elasticFieldOf(IndicatorPointEntity.Fields.timestamp);
        else if (item.match(IndicatorPoint._groupHash))
            return this.elasticFieldOf(IndicatorPointEntity.Fields.groupHash);
        else if (item.prefix(IndicatorPoint._groupInfo))
            return this.queryFactory.query(DataGroupInfoQuery.class).innerPath(IndicatorPointEntity.Fields.groupInfo).fieldNameOf(this.extractPrefixed(item, IndicatorPoint._groupInfo));
        else if (this.containsFieldEntity(item.getField()))
            return this.elasticFieldOf(this.getFieldEntity(item.getField()).getCode()).disableInfer(true);
        else if (item.getPrefix() != null && this.containsFieldEntity(item.getPrefix()))
            return this.elasticFieldOf(item.getFieldWithoutPrefix())
    ...
    protected Boolean supportsMetricAggregate(MetricAggregateType metricAggregateType, FieldResolver resolver) {
        if (resolver.match(IndicatorPoint._timestamp))
            return Boolean.TRUE;
        else if (resolver.match(IndicatorPoint._batchTimestamp))
            return Boolean.TRUE;
    ...
    protected ElasticNestedQuery<?, ?, ?> nestedQueryOf(FieldResolver item) {
        ...
        if (fieldEntity == null && item.getPrefix() != null)
            fieldEntity = this.getOrDefaultFieldEntity(item.getPrefix(), null);

        if (fieldEntity != null && fieldEntity.getBaseType() == IndicatorFieldBaseType.IntegerMap)
            return this.queryFactory.query(IndicatorIntegerMapQuery.class).nestedPath(fieldEntity.getCode());
        else if (fieldEntity != null && fieldEntity.getBaseType() == IndicatorFieldBaseType.DoubleMap)
            return this.queryFactory.query(IndicatorDoubleMapQuery.class).nestedPath(fieldEntity.getCode());
        else
            return null;
    }
    ...
    private FieldEntity getFieldEntity(String field) {
        FieldEntity fieldEntity = this.getOrDefaultFieldEntity(field, null);
        if (fieldEntity == null)
            throw new MyApplicationException("invalid field " + field);
        return fieldEntity;
    }
