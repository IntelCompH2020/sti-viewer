...
public class IndicatorPointValidationServiceImpl implements IndicatorPointValidationService {
    ...
    public void validateModel() {
        if (!validationErrors.isEmpty())
            throw new MyValidationException("validation error", validationErrors);
    }
    ...
    public void validateField(List<ValidationEntity> validations, Map.Entry<String, Object> field, IndicatorFieldBaseType baseType) {
        fieldErrors.set(new AbstractMap.SimpleEntry<>(field.getKey(), new ArrayList<>(100)));
        ...
        if (!fieldErrors.get().getValue().isEmpty())
            validationErrors.add(fieldErrors.get());
    }
    ...
    public void checkIfMissingRequiredFields(Map<String, Object> modelProps, Map<String, FieldEntity> configProps) {
        ...
        List<String> requiredFields = configProps.entrySet().stream()
                .filter(f -> f.getValue().getValidation() != null && f.getValue().getValidation()
                        .stream()
                        .anyMatch(valid -> valid.getType() == IndicatorPointValidationType.REQUIRED))
        ...
        if (!missingFields.isEmpty())
            throw new MyValidationException("Missing required fields: [ " + missingFields + " ]");
    ...
    private void checkAllowedValues(Map.Entry<String, Object> field, ValidationEntity validation, IndicatorFieldBaseType baseType) {
        ...
        switch (baseType) {
            case String:
            case Keyword:
                if (conventionService.isListNullOrEmpty(validation.getStringValues()))
                    break;
                valid = validation.getStringValues().contains((String) value);
                allowedValues = String.join(",", validation.getStringValues());
                break;
            case Date:
                if (conventionService.isListNullOrEmpty(validation.getDateValues()))
                    break;
                ...
                break;
            case Double:
                if (conventionService.isListNullOrEmpty(validation.getDoubleValues()))
                    break;
                ...
                break;
            case Integer:
                if (conventionService.isListNullOrEmpty(validation.getIntValues()))
                    break;
            ...
            case IntegerArray:
                if (conventionService.isListNullOrEmpty(validation.getIntValues()))
                    break;
                valid = new HashSet<>(validation.getIntValues()).containsAll((List<Integer>) value);
                allowedValues = validation.getIntValues().stream().map(String::valueOf).collect(Collectors.joining(","));
                break;
            case DoubleArray:
                if (conventionService.isListNullOrEmpty(validation.getDoubleValues()))
                    break;
                valid = new HashSet<>(validation.getDoubleValues()).containsAll((List<Double>) value);
                allowedValues = validation.getDoubleValues().stream().map(String::valueOf).collect(Collectors.joining(","));
                break;
            case KeywordArray:
                if (conventionService.isListNullOrEmpty(validation.getStringValues()))
                    break;
                valid = new HashSet<>(validation.getStringValues()).containsAll((List<String>) value);
                allowedValues = String.join(",", validation.getStringValues());
                ;
                break;
            case IntegerMap:
                ...
                List<Integer> collectedInts = Arrays.stream(textIntegerItems)
                        .map(txt -> Integer.valueOf(txt.split("\\|")[1]))
                        .collect(Collectors.toList());
                valid = new HashSet<>(validation.getIntValues()).containsAll(collectedInts);
                allowedValues = validation.getIntValues().stream().map(String::valueOf).collect(Collectors.joining(","));
                break;
            case DoubleMap:
                if (conventionService.isListNullOrEmpty(validation.getIntValues()))
                ...
                List<Double> collectedDoubles = Arrays.stream(textDoubleItems)
                        .map(txt -> Double.valueOf(txt.split("\\|")[1]))
                        .collect(Collectors.toList());
                valid = new HashSet<>(validation.getDoubleValues()).containsAll(collectedDoubles);
                allowedValues = validation.getDoubleValues().stream().map(String::valueOf).collect(Collectors.joining(","));
                break;
            default:
                throw new MyApplicationException("invalid type " + baseType);
        ...
        if (!valid)
            fieldErrors.get().getValue().add("Value of field " + fieldName + " is not contained in allowed values:[ " + allowedValues + " ]");
    }
    ...
    private void checkValueRange(Map.Entry<String, Object> field, ValidationEntity validation, IndicatorFieldBaseType baseType) {
        ...
        switch (baseType) {
            ...
            case IntegerMap:
                ...
                List<Integer> collectedInts = Arrays.stream(textIntegerItems)
                        .map(txt -> Integer.valueOf(txt.split("\\|")[1]))
                        .collect(Collectors.toList());
                if (this.conventionService.isListNullOrEmpty(collectedInts))
                    break;
                int intIndex2 = 0;
                while (valid && collectedInts.size() > intIndex2) {
                    valid = checkValidDoubleRange(validation, collectedInts.get(intIndex2));
                    intIndex2++;
                }
                ValueRange = integerValuesRangeToString(validation);
                break;
            case DoubleMap:
                ...
                List<Double> collectedDoubles = Arrays.stream(textDoubleItems)
                        .map(txt -> Double.valueOf(txt.split("\\|")[1]))
                        .collect(Collectors.toList());
                if (this.conventionService.isListNullOrEmpty(collectedDoubles))
                    break;
                int doubleIndex2 = 0;
                while (valid && collectedDoubles.size() > doubleIndex2) {
        ...
        if (!valid)
            fieldErrors.get().getValue().add("Value of field " + fieldName + " is not in value Range: [ " + ValueRange + " ]");
    }
