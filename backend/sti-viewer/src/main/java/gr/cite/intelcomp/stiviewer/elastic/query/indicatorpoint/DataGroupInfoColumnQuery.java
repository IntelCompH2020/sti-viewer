...
//		}
    ...
    public DataGroupInfoColumnEntity convert(Map<String, Object> rawData, Set<String> columns) {
        ...
        if (columns.contains(DataGroupInfoColumnEntity.Fields.fieldCode))
            mocDoc.setFieldCode(FieldBasedMapper.shallowSafeConversion(rawData.get(DataGroupInfoColumnEntity.Fields.fieldCode), String.class));
        if (columns.contains(DataGroupInfoColumnEntity.Fields.values))
            mocDoc.setValues(FieldBasedMapper.shallowSafeConversion(rawData.get(DataGroupInfoColumnEntity.Fields.values), mapper.getTypeFactory().constructParametricType(List.class, String.class)));
        return mocDoc;
    }

    @Override
    protected ElasticField fieldNameOf(FieldResolver item) {
        if (item.match(DataGroupInfoColumn._fieldCode))
            return this.elasticFieldOf(DataGroupInfoColumnEntity.Fields.fieldCode).disableInfer(true);
        else if (item.match(DataGroupInfoColumn._values))
            return this.elasticFieldOf(DataGroupInfoColumnEntity.Fields.values).disableInfer(true);
        else
            return null;
    }
