...
public class DynamicPageConfigBuilder extends BaseBuilder<DynamicPageConfig, DynamicPageConfigEntity> {
	...
	public DynamicPageConfigBuilder(ConventionService conventionService, BuilderFactory builderFactory) {
	...
	public List<DynamicPageConfig> build(FieldSet fields, List<DynamicPageConfigEntity> data) throws MyApplicationException {
		...
		List<DynamicPageConfig> models = new ArrayList<>();

		for (DynamicPageConfigEntity d : data) {
			DynamicPageConfig m = new DynamicPageConfig();
			if (fields.hasField(this.asIndexer(DynamicPageConfig._allowedRoles))) m.setAllowedRoles(d.getAllowedRoles());
			if (fields.hasField(this.asIndexer(DynamicPageConfig._externalUrl))) m.setExternalUrl(d.getExternalUrl());
			if (fields.hasField(this.asIndexer(DynamicPageConfig._matIcon))) m.setMatIcon(d.getMatIcon());
			models.add(m);
		}
