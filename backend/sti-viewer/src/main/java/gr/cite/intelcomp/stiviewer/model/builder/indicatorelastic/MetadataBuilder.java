...
public class MetadataBuilder extends BaseBuilder<Metadata, IndicatorMetadataEntity> {
    ...
    public MetadataBuilder(ConventionService conventionService, BuilderFactory builderFactory) {
        super(conventionService, new LoggerService(LoggerFactory.getLogger(MetadataBuilder.class)));
    ...
    @Override
    public List<Metadata> build(FieldSet fields, List<IndicatorMetadataEntity> data) throws MyApplicationException {
        ...
        List<Metadata> metadata = new ArrayList<>(100);

        if (data == null)
            return metadata;
        for (IndicatorMetadataEntity d : data) {
            Metadata m = new Metadata();
            if (fields.hasField(this.asIndexer(Metadata._code)))
                m.setCode(d.getCode());
            if (fields.hasField(this.asIndexer(Metadata._date)))
                m.setDate(d.getDate());
            if (fields.hasField(this.asIndexer(Metadata._description)))
                m.setDescription(d.getDescription());
            if (fields.hasField(this.asIndexer(Metadata._label)))
                m.setLabel(d.getLabel());
            if (fields.hasField(this.asIndexer(Metadata._url)))
                m.setUrl(d.getUrl());
            ...
            if (!coverageFields.isEmpty() && d.getCoverage() != null)
                m.setCoverage(this.builderFactory.builder(CoverageBuilder.class).authorize(this.authorize).build(coverageFields, d.getCoverage()));
            ...
            if (!semanticLabelsFields.isEmpty() && d.getSemanticLabels() != null)
                m.setSemanticLabels(this.builderFactory.builder(SemanticsLabelBuilder.class).authorize(this.authorize).build(semanticLabelsFields, d.getSemanticLabels()));
