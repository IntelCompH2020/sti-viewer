...
public class BrowseDataTreeLevelConfigBuilder extends BaseBuilder<DataTreeLevelConfig, DataTreeLevelConfigEntity> {
    ...
    public BrowseDataTreeLevelConfigBuilder(BuilderFactory builderFactory,
    ...
    public List<DataTreeLevelConfig> build(FieldSet fields, List<DataTreeLevelConfigEntity> data) throws MyApplicationException {
        ...
        List<DataTreeLevelConfig> IndicatorReportLevelConfigs = new ArrayList<>(100);
        ...
        for (DataTreeLevelConfigEntity d : data) {
            ...
            if (fields.hasField(this.asIndexer(DataTreeLevelConfig._order)))
                m.setOrder(d.getOrder());
            if (fields.hasField(this.asIndexer(DataTreeLevelConfig._supportSubLevel)))
                m.setSupportSubLevel(d.getSupportSubLevel());
            if (fields.hasField(this.asIndexer(DataTreeLevelConfig._defaultDashboards)))
                m.setDefaultDashboards(d.getDefaultDashboards());
            if (!filedFields.isEmpty())
                m.setField(this.builderFactory.builder(BrowseDataFieldBuilder.class).authorize(this.authorize).build(filedFields, d.getField()));
            if (!dashboardOverrideFields.isEmpty() && d.getDashboardOverrides() != null)
                m.setDashboardOverrides(this.builderFactory.builder(DataTreeLevelDashboardOverrideBuilder.class).authorize(this.authorize).build(dashboardOverrideFields, d.getDashboardOverrides()));
