...
public class BookmarkQuery extends QueryBase<BookmarkEntity> {
    ...
    protected <X, Y> Predicate applyAuthZ(QueryContext<X, Y> queryContext) {
        if (this.authorize.contains(AuthorizationFlags.None))
            return null;
        if (this.authorize.contains(AuthorizationFlags.Permission) && this.authService.authorize(Permission.BrowseBookmark))
            return null;
        UUID ownerId = null;
        ...
        if (this.like != null && !this.like.isEmpty()) {
            ...
            if (userSettings != null && !userSettings.isEmpty()) {
                for (UserSettingsEntity userSettingsEntity : userSettings) {
                    if (userSettingsEntity.getKey() != null && !userSettingsEntity.getKey().isBlank())
                        likes.add(queryContext.CriteriaBuilder.like(queryContext.Root.get(BookmarkEntity._value), "%" + userSettingsEntity.getKey() + "%"));
                }
            }

            predicates.add(queryContext.CriteriaBuilder.or(likes.toArray(new Predicate[0])));
        }
    ...
    protected String fieldNameOf(FieldResolver item) {
        if (item.match(Bookmark._id))
            return BookmarkEntity._id;
        else if (item.match(Bookmark._isActive))
            return BookmarkEntity._isActive;
        else if (item.match(Bookmark._type))
            return BookmarkEntity._type;
        else if (item.match(Bookmark._name))
            return BookmarkEntity._name;
        else if (item.match(Bookmark._hashCode))
            return BookmarkEntity._hashCode;
        else if (item.match(Bookmark._value))
            return BookmarkEntity._value;
        else if (item.match(Bookmark._createdAt))
            return BookmarkEntity._createdAt;
        else if (item.match(Bookmark._updatedAt))
            return BookmarkEntity._updatedAt;
        else if (item.match(Bookmark._hash))
            return BookmarkEntity._updatedAt;
        else if (item.prefix(Bookmark._user))
            return BookmarkEntity._userId;
        else
            return null;
    }
}
