...
public class UserSettingsQuery extends QueryBase<UserSettingsEntity> {
    ...
    protected <X, Y> Predicate applyAuthZ(QueryContext<X, Y> queryContext) {
        if (this.authorize.contains(AuthorizationFlags.None))
            return null;
        if (this.authorize.contains(AuthorizationFlags.Permission) && this.authService.authorize(Permission.BrowseUserSettings))
            return null;
        UUID ownerId = null;
    ...
    protected String fieldNameOf(FieldResolver item) {
        if (item.match(UserSettings._id))
            return UserSettingsEntity._id;
        else if (item.match(UserSettings._entityId))
            return UserSettingsEntity._entityId;
        else if (item.match(UserSettings._value))
            return UserSettingsEntity._value;
        else if (item.match(UserSettings._entityType))
            return UserSettingsEntity._entityType;
        else if (item.match(UserSettings._key))
            return UserSettingsEntity._key;
        else if (item.match(UserSettings._type))
            return UserSettingsEntity._type;
        else if (item.match(UserSettings._createdAt))
            return UserSettingsEntity._createdAt;
        else if (item.match(UserSettings._updatedAt))
            return UserSettingsEntity._updatedAt;
        else if (item.match(UserSettings._hash))
            return UserSettingsEntity._updatedAt;
        else
            return null;
    }
