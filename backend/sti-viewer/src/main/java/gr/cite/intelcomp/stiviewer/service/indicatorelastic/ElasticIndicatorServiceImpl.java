...
public class ElasticIndicatorServiceImpl implements ElasticIndicatorService {
    ...
    public IndicatorElastic persist(IndicatorElasticPersist persist, FieldSet fieldSet) throws IOException, InvalidApplicationException {
        ...
        if (persist.getId() != null) {
            ...
            if (this.queryFactory.query(IndicatorElasticQuery.class).ids(indicatorId).count() > 0)
                throw new IllegalStateException("Updates not allowed");
        } else {
    ...
    private void createDbIndicator(IndicatorElasticPersist persist, UUID indicatorId) throws IOException, InvalidApplicationException {
        ...
        this.indicatorService.persist(data, null, indicatorId);
    }
    ...
    private FieldEntity buildFieldEntity(FieldPersist persist) {
        ...
        if (persist.getOperations() != null && !persist.getOperations().isEmpty()) {
            List<OperationEntity> items = new ArrayList<>(100);
        ...
        if (persist.getAltLabels() != null && !persist.getAltLabels().isEmpty()) {
            List<AltTextEntity> items = new ArrayList<>(100);
    ...
    private ValueRangeEntity buildValueRangeEntity(ValueRangePersist persist) {
        ...
        if (persist.getValues() != null && !persist.getValues().isEmpty()) {
            List<ValueRangeValueEntity> items = new ArrayList<>(100);
    ...
    private IndicatorMetadataEntity buildIndicatorMetadataEntity(MetadataPersist persist) {
        ...
        if (persist.getCoverage() != null && !persist.getCoverage().isEmpty()) {
            List<CoverageEntity> items = new ArrayList<>(100);
        ...
        if (persist.getSemanticLabels() != null && !persist.getSemanticLabels().isEmpty()) {
            List<SemanticLabelEntity> items = new ArrayList<>(100);
            for (SemanticsLabelPersist item : persist.getSemanticLabels())
                items.add(this.buildSemanticsLabelPersist(item));
    ...
    public String calculateIndexName(String code, boolean ensureUnique) throws IOException {
        ...
        if (ensureUnique && restHighLevelClient.indices().exists(new GetIndexRequest(index), RequestOptions.DEFAULT))
    ...
    public String ensureIndex(UUID indicatorId) throws IOException, InvalidApplicationException {
        ...
        if (!exists) {
            ...
            restHighLevelClient.indices().create(new CreateIndexRequest(index).mapping(builder).settings(settingsBuilder), RequestOptions.DEFAULT);
        }
        return index;
    }
    ...
    public void deleteAndSave(UUID id) throws MyForbiddenException, InvalidApplicationException, IOException {
        ...
        if (data == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{id, IndicatorElastic.class.getSimpleName()}, LocaleContextHolder.getLocale()));
