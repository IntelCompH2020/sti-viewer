...
public class DataGroupRequestBuilder extends BaseBuilder<DataGroupRequest, DataGroupRequestEntity> {
    ...
    public List<DataGroupRequest> build(FieldSet fields, List<DataGroupRequestEntity> data) throws MyApplicationException {
        ...
        List<DataGroupRequest> models = new ArrayList<>(100);

        for (DataGroupRequestEntity d : data) {
            DataGroupRequest m = new DataGroupRequest();
            if (fields.hasField(this.asIndexer(DataGroupRequest._id)))
                m.setId(d.getId());
            if (fields.hasField(this.asIndexer(DataGroupRequest._status)))
                m.setStatus(d.getStatus());
            if (fields.hasField(this.asIndexer(DataGroupRequest._groupHash)))
                m.setGroupHash(d.getGroupHash());
            if (fields.hasField(this.asIndexer(DataGroupRequest._name)))
                m.setName(d.getName());
            if (fields.hasField(this.asIndexer(DataGroupRequest._isActive)))
                m.setIsActive(d.getIsActive());
            if (fields.hasField(this.asIndexer(DataGroupRequest._createdAt)))
                m.setCreatedAt(d.getCreatedAt());
            if (fields.hasField(this.asIndexer(DataGroupRequest._updatedAt)))
            ...
            if (!indicatorFields.isEmpty() && d.getConfig() != null) {
                ...
                if (dataGroupRequestConfigType != null)
                    m.setConfig(this.builderFactory.builder(DataGroupRequestConfigBuilder.class).authorize(this.authorize).build(indicatorFields, dataGroupRequestConfigType));
            }

            if (fields.hasField(this.asIndexer(DataGroupRequest._hash)))
    ...
    private Map<UUID, User> collectUsers(FieldSet fields, List<DataGroupRequestEntity> data) throws MyApplicationException {
        ...
        if (!fields.hasOtherField(this.asIndexer(User._id))) {
            itemMap = this.asEmpty(
                    data.stream().map(DataGroupRequestEntity::getUserId).distinct().collect(Collectors.toList()),
                    x -> {
                        User item = new User();
                        item.setId(x);
                        return item;
            ...
            UserQuery q = this.queryFactory.query(UserQuery.class).authorize(this.authorize).ids(data.stream().map(DataGroupRequestEntity::getUserId).distinct().collect(Collectors.toList()));
