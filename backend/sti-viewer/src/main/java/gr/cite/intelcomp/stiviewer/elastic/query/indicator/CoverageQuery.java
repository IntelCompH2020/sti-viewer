...
public class CoverageQuery extends ElasticNestedQuery<CoverageQuery, CoverageEntity, String> {
	...
	public CoverageEntity convert(Map<String, Object> rawData, Set<String> columns) {
		...
		if (columns.contains(CoverageEntity.Fields.label)) mocDoc.setLabel(FieldBasedMapper.shallowSafeConversion(rawData.get(CoverageEntity.Fields.label), String.class));
		if (columns.contains(CoverageEntity.Fields.max)) mocDoc.setMax(FieldBasedMapper.shallowSafeConversion(rawData.get(CoverageEntity.Fields.max), Double.class));
		if (columns.contains(CoverageEntity.Fields.min)) mocDoc.setMin(FieldBasedMapper.shallowSafeConversion(rawData.get(CoverageEntity.Fields.min), Double.class));
		return mocDoc;
	}
