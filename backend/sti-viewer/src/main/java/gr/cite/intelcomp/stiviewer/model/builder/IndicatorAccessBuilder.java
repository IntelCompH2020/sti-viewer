...
public class IndicatorAccessBuilder extends BaseBuilder<IndicatorAccess, IndicatorAccessEntity> {
	...
	public List<IndicatorAccess> build(FieldSet fields, List<IndicatorAccessEntity> data) throws MyApplicationException {
		...
		List<IndicatorAccess> models = new ArrayList<>();

		for (IndicatorAccessEntity d : data) {
			IndicatorAccess m = new IndicatorAccess();
			if (fields.hasField(this.asIndexer(IndicatorAccess._id))) m.setId(d.getId());
			if (fields.hasField(this.asIndexer(IndicatorAccess._isActive))) m.setIsActive(d.getIsActive());
			if (fields.hasField(this.asIndexer(IndicatorAccess._createdAt))) m.setCreatedAt(d.getCreatedAt());
			if (fields.hasField(this.asIndexer(IndicatorAccess._updatedAt))) m.setUpdatedAt(d.getUpdatedAt());
			if (fields.hasField(this.asIndexer(IndicatorAccess._hash))) m.setHash(this.hashValue(d.getUpdatedAt()));
			...
			if (!indicatorFields.isEmpty() && indicatorItemsMap != null && indicatorItemsMap.containsKey(d.getIndicatorId())) m.setIndicator(indicatorItemsMap.get(d.getIndicatorId()));
	...
	private Map<UUID, Indicator> collectIndicators(FieldSet fields, List<IndicatorAccessEntity> data) throws MyApplicationException {
		...
		if (!fields.hasOtherField(this.asIndexer(Indicator._id))) {
			itemMap = this.asEmpty(
					data.stream().map(x -> x.getIndicatorId()).distinct().collect(Collectors.toList()),
					x -> {
			...
			IndicatorQuery q = this.queryFactory.query(IndicatorQuery.class).authorize(this.authorize).ids(data.stream().map(x -> x.getIndicatorId()).distinct().collect(Collectors.toList()));
