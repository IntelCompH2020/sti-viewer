...
public class MasterItemQuery extends QueryBase<MasterItemEntity> {
	...
	protected String fieldNameOf(FieldResolver item) {
		if (item.match(MasterItem._id)) return MasterItemEntity._id;
		else if (item.match(MasterItem._name)) return MasterItemEntity._name;
		else if (item.match(MasterItem._isActive)) return MasterItemEntity._isActive;
		else if (item.match(MasterItem._createdAt)) return MasterItemEntity._createdAt;
		else if (item.match(MasterItem._updatedAt)) return MasterItemEntity._updatedAt;
		else if (item.match(MasterItem._hash)) return MasterItemEntity._updatedAt;
		else return null;
	}
}
