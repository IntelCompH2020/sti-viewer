...
public class BrowseDataTreeLevelBuilder extends BaseBuilder<DataTreeLevel, DataTreeLevelEntity> {
    ...
    public BrowseDataTreeLevelBuilder(BuilderFactory builderFactory,
    ...
    public List<DataTreeLevel> build(FieldSet fields, List<DataTreeLevelEntity> data) throws MyApplicationException {
        ...
        List<DataTreeLevel> models = new ArrayList<>(100);
        ...
        for (DataTreeLevelEntity d : data) {
            ...
            if (fields.hasField(this.asIndexer(DataTreeLevel._order)))
                m.setOrder(d.getOrder());
            if (fields.hasField(this.asIndexer(DataTreeLevel._supportSubLevel)))
                m.setSupportSubLevel(d.getSupportSubLevel());
            if (fields.hasField(this.asIndexer(DataTreeLevel._supportedDashboards)))
            ...
            if (!itemsFields.isEmpty() && d.getItems() != null)
                m.setItems(this.builderFactory.builder(BrowseDataTreeLevelItemBuilder.class).authorize(this.authorize).build(itemsFields, d.getItems()));
