...
public class IndicatorQuery extends QueryBase<IndicatorEntity> {
    ...
    protected String fieldNameOf(FieldResolver item) {
        if (item.match(Indicator._id))
            return Indicator._id;
        else if (item.match(Indicator._code))
            return Indicator._code;
        else if (item.match(Indicator._name))
            return Indicator._name;
        else if (item.match(Indicator._description))
            return Indicator._description;
        else if (item.match(Indicator._config))
            return Indicator._config;
        else if (item.prefix(Indicator._config))
            return Indicator._config;
        else if (item.match(Indicator._createdAt))
            return Indicator._createdAt;
        else if (item.match(Indicator._updatedAt))
            return Indicator._updatedAt;
        else if (item.match(Indicator._isActive))
            return Indicator._isActive;
        else
            return null;
    }
