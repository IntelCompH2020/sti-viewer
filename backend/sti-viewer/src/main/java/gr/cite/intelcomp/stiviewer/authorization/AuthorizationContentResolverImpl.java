...
public class AuthorizationContentResolverImpl implements AuthorizationContentResolver {
    ...
    private IndicatorRolesResource resolveIndicatorAffiliation(UUID indicatorId) {
        ...
        List<IndicatorAccessEntity> indicatorAccesses = this.queryFactory.query(IndicatorAccessQuery.class).isActive(IsActive.ACTIVE).
                indicatorIds(indicatorId).hasUser(Boolean.FALSE).collectAs(new BaseFieldSet().ensure(IndicatorAccess._id).ensure(this.conventionService.asIndexer(IndicatorAccess._indicator, Indicator._id)).ensure(IndicatorAccess._user));
        try {
    ...
    public List<HierarchyIndicatorColumnAccess> indicatorAllowedKeywords(UUID... indicatorIds) {
        ...
        if (cacheValue == null) {
            List<IndicatorAccessEntity> indicatorAccesses;
            List<IndicatorEntity> indicators;
            try {
                indicatorAccesses = this.queryFactory.query(IndicatorAccessQuery.class).indicatorIds(indicatorIds).hasUser(Boolean.FALSE).isActive(IsActive.ACTIVE).tenantIds(this.tenantScope.getTenant()).collectAs(new BaseFieldSet().ensure(IndicatorAccess._config).ensure(IndicatorAccess._indicator));
            ...
            for (IndicatorAccessEntity indicatorAccess : indicatorAccesses) {
                this.buildHierarchyIndicatorColumnAccesses(items, indicators, indicatorAccess);
            }
    ...
    private void buildHierarchyIndicatorColumnAccesses(List<HierarchyIndicatorColumnAccess> allHierarchyIndicatorColumnAccesses, List<IndicatorEntity> indicators, IndicatorAccessEntity indicatorAccess) {
        ...
        if (indicatorAccessConfig == null || indicatorAccessConfig.getAllFilterColumns() == null || indicatorAccessConfig.getAllFilterColumns().isEmpty())
            return;

        List<IndicatorColumnAccess> tempFilterColumns = indicatorAccessConfig.getAllFilterColumns().stream().map(x -> {
        ...
        if (indicator == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{indicatorAccess.getIndicatorId(), Indicator.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private HierarchyIndicatorColumnAccess buildHierarchyIndicatorColumnAccess(IndicatorEntity indicator, List<String> fieldsHierarchy, int level, String code, String value) {
        ...
        if (levelCode.equals(code)) {
            ...
            if (distinctValuesResponse.getItems() == null || distinctValuesResponse.getItems().isEmpty())
                throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{code, FilterColumnConfigEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
            ...
            if (this.conventionService.isNullOrEmpty(fieldHierarchyValue))
                throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{code, FilterColumnConfigEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private List<UUID> resolveAffiliatedIndicators(String... permissions) {
        ...
        List<IndicatorAccessEntity> indicatorAccesses = this.queryFactory.query(IndicatorAccessQuery.class).isActive(IsActive.ACTIVE).hasUser(Boolean.FALSE).collectAs(new BaseFieldSet().ensure(IndicatorAccess._id).ensure(this.conventionService.asIndexer(IndicatorAccess._indicator, Indicator._id)).ensure(IndicatorAccess._user));
        try {
