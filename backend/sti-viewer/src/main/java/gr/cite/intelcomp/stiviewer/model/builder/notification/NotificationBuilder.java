...
public class NotificationBuilder extends BaseBuilder<Notification, NotificationEntity> {
    ...
    public NotificationBuilder(
        ...
        super(conventionService, new LoggerService(LoggerFactory.getLogger(NotificationBuilder.class)));
    ...
    public List<Notification> build(FieldSet fields, List<NotificationEntity> data) throws MyApplicationException {
        ...
        List<Notification> models = new ArrayList<>(100);

        for (NotificationEntity d : data) {
            Notification m = new Notification();
            if (fields.hasField(this.asIndexer(Notification._id)))
                m.setId(d.getId());
            if (fields.hasField(this.asIndexer(Notification._type)))
                m.setType(d.getType());
            if (fields.hasField(this.asIndexer(Notification._contactHint)))
                m.setContactHint(d.getContactHint());
            if (fields.hasField(this.asIndexer(Notification._contactTypeHint)))
                m.setContactTypeHint(d.getContactTypeHint());
            if (fields.hasField(this.asIndexer(Notification._data)))
                m.setData(d.getData());
            if (fields.hasField(this.asIndexer(Notification._notifyState)))
                m.setNotifyState(d.getNotifyState());
            if (fields.hasField(this.asIndexer(Notification._notifiedWith)))
                m.setNotifiedWith(d.getNotifiedWith());
            if (fields.hasField(this.asIndexer(Notification._notifiedAt)))
                m.setNotifiedAt(d.getNotifiedAt());
            if (fields.hasField(this.asIndexer(Notification._retryCount)))
                m.setRetryCount(d.getRetryCount());
            if (fields.hasField(this.asIndexer(Notification._trackingState)))
                m.setTrackingState(d.getTrackingState());
            if (fields.hasField(this.asIndexer(Notification._trackingProcess)))
                m.setTrackingProcess(d.getTrackingProcess());
            if (fields.hasField(this.asIndexer(Notification._trackingData)))
                m.setTrackingData(d.getTrackingData());
            if (fields.hasField(this.asIndexer(Notification._provenanceRef)))
                m.setProvenanceRef(d.getProvenanceRef());
            if (fields.hasField(this.asIndexer(Notification._notifiedAt)))
                m.setNotifiedAt(d.getNotifiedAt());
            if (fields.hasField(this.asIndexer(Notification._isActive)))
                m.setIsActive(d.getIsActive());
            if (fields.hasField(this.asIndexer(Notification._createdAt)))
                m.setCreatedAt(d.getCreatedAt());
            if (fields.hasField(this.asIndexer(Notification._updatedAt)))
                m.setUpdatedAt(d.getUpdatedAt());
            if (fields.hasField(this.asIndexer(Notification._hash)))
