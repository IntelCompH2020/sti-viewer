...
public class DynamicPageBuilder extends BaseBuilder<DynamicPage, DynamicPageEntity> {
	...
	@Override
	public List<DynamicPage> build(FieldSet fields, List<DynamicPageEntity> datas) throws MyApplicationException {
		...
		List<DynamicPage> models = new ArrayList<>();

		FieldSet creatorFields = fields.extractPrefixed(this.asPrefix(DynamicPage._creator));
		Map<UUID, User> creatorItemsMap = this.collectCreators(creatorFields, datas);

		FieldSet dynamicPageContentsFields = fields.extractPrefixed(this.asPrefix(DynamicPage._pageContents));
		Map<UUID, List<DynamicPageContent>> dynamicPageContentsMap = this.collectPageContents(dynamicPageContentsFields, datas);
		// TODO make it in bulk
		FieldSet dynamicPageConfigFields = fields.extractPrefixed(this.asPrefix(DynamicPage._config));

		for (DynamicPageEntity d : datas) {
			DynamicPage m = new DynamicPage();
			if (fields.hasField(this.asIndexer(DynamicPage._id))) m.setId(d.getId());
			if (fields.hasField(this.asIndexer(DynamicPage._visibility))) m.setVisibility(d.getVisibility());
			if (fields.hasField(this.asIndexer(DynamicPage._order))) m.setOrder(d.getOrder());
			if (fields.hasField(this.asIndexer(DynamicPage._type))) m.setType(d.getType());
			if (fields.hasField(this.asIndexer(DynamicPage._defaultLanguage))) m.setDefaultLanguage(d.getDefaultLanguage());
			if (!dynamicPageConfigFields.isEmpty() && d.getConfig() != null) {
				...
				if (dynamicPageConfigEntity != null) m.setConfig(this.builderFactory.builder(DynamicPageConfigBuilder.class).authorize(this.authorize).build(dynamicPageConfigFields, dynamicPageConfigEntity));
			}
			if (fields.hasField(this.asIndexer(DynamicPage._createdAt))) m.setCreatedAt(d.getCreatedAt());
			if (fields.hasField(this.asIndexer(DynamicPage._updatedAt))) m.setUpdatedAt(d.getUpdatedAt());
			if (fields.hasField(this.asIndexer(DynamicPage._isActive))) m.setIsActive(d.getIsActive());
			if (fields.hasField(this.asIndexer(DynamicPage._hash))) m.setHash(this.hashValue(d.getUpdatedAt()));
			if (!creatorFields.isEmpty() && creatorItemsMap != null && d.getCreatorId() != null && creatorItemsMap.containsKey(d.getCreatorId())) m.setCreator(creatorItemsMap.get(d.getCreatorId()));
			if (!dynamicPageContentsFields.isEmpty() && dynamicPageContentsMap != null && dynamicPageContentsMap.containsKey(d.getId())) m.setPageContents(dynamicPageContentsMap.get(d.getId()));
			models.add(m);
		}
	...
	private Map<UUID, User> collectCreators(FieldSet fields, List<DynamicPageEntity> data) throws MyApplicationException {
		...
		if (!fields.hasOtherField(this.asIndexer(User._id))) {
			itemMap = this.asEmpty(
					data.stream().filter(x-> x.getCreatorId() != null).map(x -> x.getCreatorId()).distinct().collect(Collectors.toList()),
					x -> {
						User item = new User();
						item.setId(x);
						return item;
			...
			UserQuery q = this.queryFactory.query(UserQuery.class).authorize(this.authorize).ids(data.stream().filter(x-> x.getCreatorId() != null).map(x -> x.getCreatorId()).distinct().collect(Collectors.toList()));
	...
	private Map<UUID, List<DynamicPageContent>> collectPageContents(FieldSet fields, List<DynamicPageEntity> datas) throws MyApplicationException {
		if (fields.isEmpty() || datas.isEmpty()) return null;
		this.logger.debug("checking related - {}", DynamicPageContent.class.getSimpleName());

		Map<UUID, List<DynamicPageContent>> itemMap = null;
		FieldSet clone = new BaseFieldSet(fields.getFields()).ensure(this.asIndexer(DynamicPageContent._page, DynamicPage._id));
		DynamicPageContentQuery query = this.queryFactory.query(DynamicPageContentQuery.class).authorize(this.authorize).pageIds(datas.stream().map(x -> x.getId()).distinct().collect(Collectors.toList()));
		...
		if (!fields.hasField(this.asIndexer(DynamicPageContent._page, DynamicPage._id))) {
			itemMap.values().stream().flatMap(List::stream).filter(x -> x != null && x.getPage() != null).map(x -> {
				x.getPage().setId(null);
				return x;
