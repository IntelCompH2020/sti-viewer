...
//			Subquery<DynamicPageContentEntity> subQuery = queryContext.Query.subquery(DynamicPageContentEntity.class);
//			this.applySubQuery(this.queryFactory.query(DynamicPageContentQuery.class).like(this.like), queryContext.CriteriaBuilder, subQuery);
//			predicates.add(queryContext.CriteriaBuilder.in(queryContext.Root.get(DynamicPage._id)).value(subQuery.select(queryContext.Query.from(DynamicPageContentEntity.class).get(DynamicPageContentEntity._pageId))));
//		}
	...
	protected String fieldNameOf(FieldResolver item) {
		if (item.match(DynamicPage._id)) return DynamicPageEntity._id;
		else if (item.prefix(DynamicPage._creator)) return DynamicPageEntity._creatorId;
		else if (item.match(DynamicPage._visibility)) return DynamicPageEntity._visibility;
		else if (item.match(DynamicPage._order)) return DynamicPageEntity._order;
		else if (item.match(DynamicPage._type)) return DynamicPageEntity._type;
		else if (item.match(DynamicPage._defaultLanguage)) return DynamicPageEntity._defaultLanguage;
		else if (item.match(DynamicPage._config)) return DynamicPageEntity._config;
		else if (item.prefix(DynamicPage._config)) return DynamicPageEntity._config;
		else if (item.match(DynamicPage._createdAt)) return DynamicPageEntity._createdAt;
		else if (item.match(DynamicPage._updatedAt)) return DynamicPageEntity._updatedAt;
		else if (item.match(DynamicPage._hash)) return DynamicPageEntity._updatedAt;
		else if (item.match(DynamicPage._isActive)) return DynamicPageEntity._isActive;
		else return null;
	}
