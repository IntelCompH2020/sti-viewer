...
public class DataTreeConfigServiceImpl implements DataTreeConfigService {
    ...
    public List<DataTreeConfig> getMyConfigByKey(String key, FieldSet fields) throws InvalidApplicationException {
        ...
        if (userSettingsEntity == null && this.tenantScope.isSet())
            userSettingsEntity = this.queryFactory.query(UserSettingsQuery.class).types(UserSettingsType.BrowseDataTree).entityTypes(UserSettingsEntityType.Tenant).entityIds(this.tenantScope.getTenant()).keys(key).firstAs(new BaseFieldSet().ensure(UserSettings._key).ensure(UserSettings._value));
        if (userSettingsEntity == null)
            userSettingsEntity = this.queryFactory.query(UserSettingsQuery.class).types(UserSettingsType.BrowseDataTree).entityTypes(UserSettingsEntityType.Application).keys(key).firstAs(new BaseFieldSet().ensure(UserSettings._key).ensure(UserSettings._value));
        if (userSettingsEntity == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{key, UserSettingsEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private List<DataTreeConfigEntity> getMyDataTreeConfigs() throws InvalidApplicationException {
        ...
        if ((userSettingsEntities == null || userSettingsEntities.isEmpty()) && this.tenantScope.isSet())
            userSettingsEntities = this.queryFactory.query(UserSettingsQuery.class).types(UserSettingsType.BrowseDataTree).entityTypes(UserSettingsEntityType.Tenant).entityIds(this.tenantScope.getTenant()).collectAs(new BaseFieldSet().ensure(UserSettings._key).ensure(UserSettings._value));
        if (userSettingsEntities == null || userSettingsEntities.isEmpty())
            userSettingsEntities = this.queryFactory.query(UserSettingsQuery.class).types(UserSettingsType.BrowseDataTree).entityTypes(UserSettingsEntityType.Application).collectAs(new BaseFieldSet().ensure(UserSettings._key).ensure(UserSettings._value));
        ...
        if (userSettingsEntities != null) {
            for (UserSettingsEntity userSettingsEntity : userSettingsEntities) {
                DataTreeConfigEntity[] dataTreeConfigs = this.jsonHandlingService.fromJsonSafe(DataTreeConfigEntity[].class, userSettingsEntity.getValue());
                if (dataTreeConfigs != null) {
                    browseDataTreeConfigEntities.addAll(Arrays.asList(dataTreeConfigs));
                }
            }
        }
    ...
    private DataTreeLastViewConfigEntity getDataTreeLastViewConfig(String configId) {
        List<UserSettingsEntity> userSettingsEntities;
        try {
        ...
        if (userSettingsEntities != null) {
            for (UserSettingsEntity userSettingsEntity : userSettingsEntities) {
                return this.jsonHandlingService.fromJsonSafe(DataTreeLastViewConfigEntity.class, userSettingsEntity.getValue());
            }
        }
        return null;
    }
    ...
    public DataTreeLevel getIndicatorReportLevel(IndicatorReportLevelLookup lookup, FieldSet fields) throws InvalidApplicationException {
        ...
        if (viewConfig == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{lookup.getConfigId(), DataTreeConfigEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
        ...
        for (DataTreeLevelConfigEntity dataTreeLevelConfigEntity : viewConfig.getLevelConfigs().stream().sorted(Comparator.comparingInt(DataTreeLevelConfigEntity::getOrder)).collect(Collectors.toList())) {
            if (lookup.getSelectedLevels() == null || lookup.getSelectedLevels().stream().noneMatch(x -> x.equals(dataTreeLevelConfigEntity.getField().getCode()))) {
                nextLevelConfig = dataTreeLevelConfigEntity;
                break;
            }
        }
        if (nextLevelConfig == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{lookup.getConfigId(), DataTreeLevelConfigEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));

        DataTreeLastViewConfigEntity dataTreeLastViewConfig = this.getDataTreeLastViewConfig(lookup.getConfigId());
        if (dataTreeLastViewConfig == null && !this.conventionService.isNullOrEmpty(lookup.getParentConfigId()))
            dataTreeLastViewConfig = this.getDataTreeLastViewConfig(lookup.getParentConfigId());
        ...
        if (this.authorizationService.authorize(Permission.BrowseIndicatorPoint)) {
            ...
            indicatorIds = indicators.stream().map(IndicatorEntity::getId).distinct().collect(Collectors.toList());
        } else {
            List<IndicatorAccessEntity> indicatorAccesses = this.queryFactory.query(IndicatorAccessQuery.class).isActive(IsActive.ACTIVE).hasUser(Boolean.FALSE).indicatorIds(indicatorGroupEntity == null ? null : indicatorGroupEntity.getIndicatorIds()).collectAs(new BaseFieldSet().ensure(this.conventionService.asIndexer(IndicatorAccess._indicator, Indicator._id), IndicatorAccess._id));
            ...
            indicatorIds = indicatorAccesses.stream().map(IndicatorAccessEntity::getIndicatorId).distinct().collect(Collectors.toList());
        }
        ...
        for (UUID indicatorId : indicatorIds) {
            ...
            for (DataTreeLevelItemEntity itemEntity : itemEntities) {
                ...
                else if (itemEntity.getHasNewData()) dataTreeLevelItemEntity.setHasNewData(Boolean.TRUE);
            }
        }
    ...
    private List<DataTreeLevelItemEntity> collectBrowseDataTreeLevelItems(IndicatorReportLevelLookup lookup, UUID indicatorId, DataTreeLevelConfigEntity nextLevelConfig, DataTreeLastViewConfigEntity lastViewItemConfig) {
        List<DataTreeLevelItemEntity> indicatorReportConfigs = new ArrayList<>(100);
        ...
        if (!lookup.getSelectedLevels().isEmpty()) {
            List<String> notFoundFilters;
            if (lookup.getFilters() == null || lookup.getFilters().getKeywordFilters() == null) {
                notFoundFilters = lookup.getSelectedLevels();
            } else {
                List<String> keywordFiltersFields = lookup.getFilters().getKeywordFilters().stream().map(IndicatorPointKeywordFilter::getField).collect(Collectors.toList());
                notFoundFilters = lookup.getSelectedLevels().stream().filter(x -> !keywordFiltersFields.contains(x)).collect(Collectors.toList());
            }
            if (notFoundFilters != null && !notFoundFilters.isEmpty())
                throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{String.join(",", notFoundFilters), "SelectedLevels"}, LocaleContextHolder.getLocale()));
        }
        ...
        do {
            ...
            for (AggregateResponseItem item : aggregateResponse.getItems()) {
                if (item.getGroup() == null || item.getGroup().getItems() == null || !item.getGroup().getItems().containsKey(distinctKey))
                    continue;
                ...
                AggregateResponseValue maxTimestampValue = item.getValues() != null ? item.getValues().stream().filter(x -> x.getAggregateType() == MetricAggregateType.Max && IndicatorPointEntity.Fields.timestamp.equals(x.getField())).findFirst().orElse(null) : null;
                Instant maxTimestamp = Instant.now();
                if (maxTimestampValue != null)
                    maxTimestamp = Instant.ofEpochMilli(maxTimestampValue.getValue().longValue());
                ...
                if (nextLevelConfig.getDashboardOverrides() != null) {
                    ...
                    if (!dashboardOverrideEntities.isEmpty()) {
                        levelConfigItem.setSupportedDashboards(dashboardOverrideEntities.stream().map(DataTreeLevelDashboardOverrideEntity::getSupportedDashboards).flatMap(List::stream).distinct().collect(Collectors.toList()));

                        if (dashboardOverrideEntities.stream().anyMatch(x -> x.getSupportSubLevel() != null)) {
                            levelConfigItem.setSupportSubLevel(dashboardOverrideEntities.stream().map(DataTreeLevelDashboardOverrideEntity::getSupportSubLevel).filter(Objects::nonNull).findFirst().orElse(null));
                        }
                    }
                }
