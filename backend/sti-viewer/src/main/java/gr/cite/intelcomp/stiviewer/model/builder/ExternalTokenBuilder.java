...
public class ExternalTokenBuilder extends BaseBuilder<ExternalToken, ExternalTokenEntity> {
    ...
    @Override
    public List<ExternalToken> build(FieldSet fields, List<ExternalTokenEntity> data) throws MyApplicationException {
        this.logger.debug("building for {} items requesting {} fields", Optional.ofNullable(data).map(List::size).orElse(0), Optional.ofNullable(fields).map(FieldSet::getFields).map(Set::size).orElse(0));
        this.logger.trace(new DataLogEntry("requested fields", fields));
        if (fields == null || data == null || fields.isEmpty())
            return new ArrayList<>();

        List<ExternalToken> models = new ArrayList<>(100);
        ...
        Map<UUID, User> ownerItemsMap = this.collectOwners(ownerFields, data);


        for (ExternalTokenEntity d : data) {
            ExternalToken m = new ExternalToken();
            if (fields.hasField(this.asIndexer(ExternalToken._id)))
                m.setId(d.getId());
            if (fields.hasField(this.asIndexer(ExternalToken._expiresAt)))
                m.setExpiresAt(d.getExpiresAt());
            if (fields.hasField(this.asIndexer(ExternalToken._name)))
                m.setName(d.getName());
            if (fields.hasField(this.asIndexer(ExternalToken._createdAt)))
                m.setCreatedAt(d.getCreatedAt());
            if (fields.hasField(this.asIndexer(ExternalToken._updatedAt)))
                m.setUpdatedAt(d.getUpdatedAt());
            if (fields.hasField(this.asIndexer(ExternalToken._isActive)))
                m.setIsActive(d.getIsActive());
            if (fields.hasField(this.asIndexer(ExternalToken._type)))
                m.setType(d.getType());
            if (fields.hasField(this.asIndexer(ExternalToken._hash)))
            ...
            if (!ownerFields.isEmpty() && ownerItemsMap != null && ownerItemsMap.containsKey(d.getOwnerId()))
                m.setOwner(ownerItemsMap.get(d.getOwnerId()));
            models.add(m);
        }
    ...
    private Map<UUID, User> collectOwners(FieldSet fields, List<ExternalTokenEntity> data) throws MyApplicationException {
        ...
        if (!fields.hasOtherField(this.asIndexer(User._id))) {
            itemMap = this.asEmpty(
                    data.stream().map(ExternalTokenEntity::getOwnerId).distinct().collect(Collectors.toList()),
                    x -> {
                        User item = new User();
                        item.setId(x);
                        return item;
            ...
            UserQuery q = this.queryFactory.query(UserQuery.class).authorize(this.authorize).ids(data.stream().map(ExternalTokenEntity::getOwnerId).distinct().collect(Collectors.toList()));
