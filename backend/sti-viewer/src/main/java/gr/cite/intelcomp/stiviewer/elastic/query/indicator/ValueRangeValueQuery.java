...
//		}
    ...
    public ValueRangeValueEntity convert(Map<String, Object> rawData, Set<String> columns) {
        ...
        if (columns.contains(ValueRangeValueEntity.Fields.value))
            mocDoc.setValue(FieldBasedMapper.shallowSafeConversion(rawData.get(ValueRangeValueEntity.Fields.value), String.class));
        if (columns.contains(ValueRangeValueEntity.Fields.label))
            mocDoc.setLabel(FieldBasedMapper.shallowSafeConversion(rawData.get(ValueRangeValueEntity.Fields.label), String.class));
        return mocDoc;
    }

    @Override
    protected ElasticField fieldNameOf(FieldResolver item) {
        if (item.match(ValueRangeValue._value))
            return this.elasticFieldOf(ValueRangeValueEntity.Fields.value).disableInfer(true);
        else if (item.match(ValueRangeValue._label))
            return this.elasticFieldOf(ValueRangeValueEntity.Fields.label).disableInfer(true);
        else
            return null;
    }
