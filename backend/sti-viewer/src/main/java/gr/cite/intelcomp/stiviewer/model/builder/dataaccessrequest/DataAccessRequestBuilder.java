...
public class DataAccessRequestBuilder extends BaseBuilder<DataAccessRequest, DataAccessRequestEntity> {
    ...
    public List<DataAccessRequest> build(FieldSet fields, List<DataAccessRequestEntity> data) throws MyApplicationException {
        ...
        List<DataAccessRequest> models = new ArrayList<>(100);

        for (DataAccessRequestEntity d : data) {
            DataAccessRequest m = new DataAccessRequest();
            if (fields.hasField(this.asIndexer(DataAccessRequest._id)))
                m.setId(d.getId());
            if (fields.hasField(this.asIndexer(DataAccessRequest._status)))
                m.setStatus(d.getStatus());
            if (fields.hasField(this.asIndexer(DataAccessRequest._createdAt)))
                m.setCreatedAt(d.getCreatedAt());
            if (fields.hasField(this.asIndexer(DataAccessRequest._updatedAt)))
            ...
            if (!indicatorFields.isEmpty() && d.getConfig() != null) {
                ...
                if (dataAccessRequestConfigType != null)
                    m.setConfig(this.builderFactory.builder(DataAccessRequestConfigBuilder.class).authorize(this.authorize).build(indicatorFields, dataAccessRequestConfigType));
            }

            if (fields.hasField(this.asIndexer(DataAccessRequest._hash)))
    ...
    private Map<UUID, User> collectUsers(FieldSet fields, List<DataAccessRequestEntity> data) throws MyApplicationException {
        ...
        if (!fields.hasOtherField(this.asIndexer(User._id))) {
            itemMap = this.asEmpty(
                    data.stream().map(DataAccessRequestEntity::getUserId).distinct().collect(Collectors.toList()),
                    x -> {
                        User item = new User();
                        item.setId(x);
                        return item;
