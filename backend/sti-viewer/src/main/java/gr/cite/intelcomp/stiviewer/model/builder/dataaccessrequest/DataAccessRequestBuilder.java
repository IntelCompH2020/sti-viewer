...
public class DataAccessRequestBuilder extends BaseBuilder<DataAccessRequest, DataAccessRequestEntity> {
	...
	@Autowired
	public DataAccessRequestBuilder(
			ConventionService conventionService,
			QueryFactory queryFactory, BuilderFactory builderFactory, JsonHandlingService jsonHandlingService) {
		super(conventionService, new LoggerService(LoggerFactory.getLogger(DataAccessRequestBuilder.class)));
		this.queryFactory = queryFactory;
		this.builderFactory = builderFactory;
		this.jsonHandlingService = jsonHandlingService;
	}

	public DataAccessRequestBuilder authorize(EnumSet<AuthorizationFlags> values) {
		this.authorize = values;
		return this;
	}

	@Override
	public List<DataAccessRequest> build(FieldSet fields, List<DataAccessRequestEntity> data) throws MyApplicationException {
		this.logger.debug("building for {} items requesting {} fields", Optional.ofNullable(data).map(List::size).orElse(0), Optional.ofNullable(fields).map(FieldSet::getFields).map(Set::size).orElse(0));
		this.logger.trace(new DataLogEntry("requested fields", fields));
		if (fields == null || data == null || fields.isEmpty()) return new ArrayList<>();

		FieldSet tenantFields = fields.extractPrefixed(this.asPrefix(DataAccessRequest._tenant));
		Map<UUID, Tenant> tenantItemsMap = this.collectTenants(tenantFields, data);

		FieldSet userFields = fields.extractPrefixed(this.asPrefix(DataAccessRequest._user));
		Map<UUID, User> userItemsMap = this.collectUsers(userFields, data);

		// TODO make it in bulk
		FieldSet indicatorFields = fields.extractPrefixed(this.asPrefix(DataAccessRequest._config));

		List<DataAccessRequest> models = new ArrayList<>();

		for (DataAccessRequestEntity d : data) {
			DataAccessRequest m = new DataAccessRequest();
			if (fields.hasField(this.asIndexer(DataAccessRequest._id))) m.setId(d.getId());
			if (fields.hasField(this.asIndexer(DataAccessRequest._status))) m.setStatus(d.getStatus());
			if (fields.hasField(this.asIndexer(DataAccessRequest._createdAt))) m.setCreatedAt(d.getCreatedAt());
			if (fields.hasField(this.asIndexer(DataAccessRequest._updatedAt))) m.setUpdatedAt(d.getUpdatedAt());
			if (!indicatorFields.isEmpty() && d.getConfig() != null) {
				DataAccessRequestConfigEntity dataAccessRequestConfigType = this.jsonHandlingService.fromJsonSafe(DataAccessRequestConfigEntity.class, d.getConfig());
				if (dataAccessRequestConfigType != null) m.setConfig(this.builderFactory.builder(DataAccessRequestConfigBuilder.class).authorize(this.authorize).build(indicatorFields, dataAccessRequestConfigType));
			}

			if (fields.hasField(this.asIndexer(DataAccessRequest._hash))) m.setHash(this.hashValue(d.getUpdatedAt()));
	...
	private Map<UUID, Tenant> collectTenants(FieldSet fields, List<DataAccessRequestEntity> data) throws MyApplicationException {
	...
	private Map<UUID, User> collectUsers(FieldSet fields, List<DataAccessRequestEntity> data) throws MyApplicationException {
		if (fields.isEmpty() || data.isEmpty()) return null;
