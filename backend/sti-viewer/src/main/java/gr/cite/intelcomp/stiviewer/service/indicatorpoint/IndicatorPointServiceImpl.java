...
public class IndicatorPointServiceImpl implements IndicatorPointService {
    ...
    public void persist(UUID indicatorId, List<IndicatorPointPersist> models) throws MyForbiddenException, MyValidationException, MyApplicationException, MyNotFoundException, InvalidApplicationException, IOException {
        ...
        IndicatorConfigItem indicatorConfigItem = this.getIndicatorConfigItem(indicatorId);
        List<IndicatorPointEntity> items = new ArrayList<>(100);
    ...
    private IndicatorConfigItem getIndicatorConfigItem(UUID indicatorId) throws InvalidApplicationException {
        ...
        if (indicatorConfigItem == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{indicatorId, IndicatorConfigItem.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private IndicatorPointEntity build(UUID indicatorId, IndicatorConfigItem indicatorConfigItem, IndicatorPointPersist model, Date now) {
        ...
        if (model.getGroupInfo() != null && model.getGroupInfo().getColumns() != null && !model.getGroupInfo().getColumns().isEmpty()) {
            ...
            groupInfo.setColumns(new ArrayList<>(100));
        ...
        if (model.getProperties() != null && indicatorConfigItem.getExtraProps() != null) {
            ...
            for (Map.Entry<String, Object> prop : model.getProperties().entrySet()) {
                if (indicatorConfigItem.getExtraProps().containsKey(prop.getKey())) {
                    ...
                    if (fieldEntity.getValidation() != null)
                        this.validationService.validateField(fieldEntity.getValidation(), prop, fieldEntity.getBaseType());
                    switch (fieldEntity.getBaseType()) {
                        ...
                        case Double:
                            if (prop.getValue() == null) {
                                ...
                                if (prop.getValue().getClass().equals(Integer.class))
                                    properties.put(prop.getKey(), Double.valueOf((Integer) prop.getValue()));
                        ...
                        case DoubleMap: {
                            if (prop.getValue() == null) {
                                ...
                                List<IndicatorDoubleMapEntity> values = new ArrayList<>(100);
                        ...
                        case IntegerMap: {
                            if (prop.getValue() == null) {
                                ...
                                List<IndicatorIntegerMapEntity> values = new ArrayList<>(100);
    ...
    public AggregateResponseModel reportPublic(@NotNull UUID indicatorId, @NotNull PublicIndicatorPointReportLookup lookup, FieldSet fields) throws InvalidApplicationException, NoSuchAlgorithmException, OperationNotSupportedException {
        ...
        if (tenant == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{tokenEntity.getTenantId(), TenantEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private AggregateResponse report(UUID indicatorId, IndicatorPointReportLookup lookup, IndicatorPointQueryDefinitionEntity definitionEntity) throws InvalidApplicationException {
        ...
        if (!indicatorPointQuery.appliesToIndicator(indicatorId))
            throw new MyForbiddenException("Access is denied");
    ...
    private AggregateResponse report(IndicatorPointQuery indicatorPointQuery, IndicatorConfigItem indicatorConfigItem, IndicatorPointReportLookup lookup) {
        ...
        if (!isRawData) {
            ...
            if (lookup.getRawDataRequest() == null)
                throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{RawDataRequest.class.getSimpleName(), IndicatorPointReportLookup.class.getSimpleName()}, LocaleContextHolder.getLocale()));
            if (lookup.getRawDataRequest().getOrder() != null)
                indicatorPointQuery.setOrder(lookup.getRawDataRequest().getOrder());
            if (lookup.getRawDataRequest().getPage() != null)
                indicatorPointQuery.setPage(lookup.getRawDataRequest().getPage());
    ...
    public byte[] exportXlsx(UUID indicatorId, IndicatorPointReportLookup lookup) throws InvalidApplicationException, IOException {
        ...
        if (lookup.getBucket() != null) {
            if (lookup.getBucket().getType() == BucketAggregateType.Nested) {
                ...
                if (this.addBucketSheet(workbook, indicatorConfigItem, nested.getBucket(), aggregateResponse, sheetIndex))
                    sheetIndex++;
                if (this.addMetricSheet(workbook, indicatorConfigItem, nested.getMetrics(), aggregateResponse, sheetIndex))
                    sheetIndex++;
            } else {
                if (this.addBucketSheet(workbook, indicatorConfigItem, lookup.getBucket(), aggregateResponse, sheetIndex))
                    sheetIndex++;
            }
        }

        this.addMetricSheet(workbook, indicatorConfigItem, lookup.getMetrics(), aggregateResponse, sheetIndex);
    ...
    private boolean addBucketSheet(Workbook workbook, IndicatorConfigItem indicatorConfigItem, gr.cite.intelcomp.stiviewer.model.elasticreport.Bucket bucket, AggregateResponse aggregateResponse, int sheetIndex) {
        ...
        switch (bucket.getType()) {
            ...
            case Terms: {
                ...
                if (fieldEntity == null)
                    throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{bucket.getField(), FieldEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
            ...
            case Composite: {
                ...
                if (composite.getDateHistogramSource() != null) {
                    ...
                    if (fieldEntity == null)
                        throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{composite.getDateHistogramSource().getField(), FieldEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
                ...
                if (composite.getSources() != null) {
                    for (gr.cite.intelcomp.stiviewer.model.elasticreport.CompositeSource source : composite.getSources()) {
                        ...
                        if (fieldEntity == null)
                            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{source.getField(), FieldEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private boolean addDataRow(Map<String, Integer> columnMapping, AggregateResponseItem item, Sheet sheet, int rowIndex) {
        ...
        if (itemColumnSize != columnMapping.size())
            return false;
    ...
    public byte[] exportJson(UUID indicatorId, IndicatorPointReportLookup lookup) throws InvalidApplicationException, UnsupportedEncodingException {
        ...
        if (lookup.getBucket() != null) {
            if (lookup.getBucket().getType() == BucketAggregateType.Nested) {
                ...
                if (bucketMetricJsonArrayBuilder != null)
                    objectBuilder.put("data_metric", bucketMetricJsonArrayBuilder);
            } else {
    ...
    private JSONArray createJsonFromBucket(IndicatorConfigItem indicatorConfigItem, gr.cite.intelcomp.stiviewer.model.elasticreport.Bucket bucket, AggregateResponse aggregateResponse) {
        ...
        ArrayList<String> columns = new ArrayList<>(100);
    ...
    private ArrayList<String> addMetricsColumns(List<gr.cite.intelcomp.stiviewer.model.elasticreport.Metric> metrics, IndicatorConfigItem indicatorConfigItem, ArrayList<String> columns) {
        if (metrics != null) {
            for (gr.cite.intelcomp.stiviewer.model.elasticreport.Metric metric : metrics) {
                ...
                if (fieldEntity == null)
                    throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{metric.getField(), FieldEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
    ...
    private List<AggregationMetricSortField> buildMetricSortFields(List<gr.cite.intelcomp.stiviewer.model.elasticreport.AggregationMetricSortField> sortFields) {
        if (sortFields == null || sortFields.isEmpty())
            return null;
        ArrayList<AggregationMetricSortField> aggregationMetricSortFields = new ArrayList<>(100);
    ...
    private BucketAggregate buildCompositeBucketAggregate(Function<String, IndicatorFieldBaseType> indicatorFieldBaseTypeFunction, gr.cite.intelcomp.stiviewer.model.elasticreport.Composite bucket) {
        if (bucket == null)
            return null;
        List<CompositeSource> sources = new ArrayList<>(100);
            ...
            for (gr.cite.intelcomp.stiviewer.model.elasticreport.CompositeSource sourceModel : bucket.getSources()) {
                switch (indicatorFieldBaseTypeFunction.apply(sourceModel.getField())) {
                    ...
                    case Keyword:
                        ...
                        source.setMissingBucket(Boolean.TRUE);
    ...
    private List<Metric> buildMetricsForQuery(Function<String, IndicatorFieldBaseType> indicatorFieldBaseTypeFunction, List<gr.cite.intelcomp.stiviewer.model.elasticreport.Metric> metricModels) {
        ...
        List<Metric> metrics = new ArrayList<>(100);
    ...
    private IndicatorFieldBaseType getIndicatorFieldBaseType(IndicatorConfigItem indicatorConfigItem, String fieldName) {
        if (IndicatorPoint._id.equals(fieldName))
            return IndicatorFieldBaseType.Keyword;
        else if (IndicatorPoint._batchId.equals(fieldName))
            return IndicatorFieldBaseType.Keyword;
        else if (IndicatorPoint._batchTimestamp.equals(fieldName))
            return IndicatorFieldBaseType.Date;
        else if (IndicatorPoint._timestamp.equals(fieldName))
            return IndicatorFieldBaseType.Date;
        else {
            FieldEntity fieldEntity = indicatorConfigItem.getExtraProps().getOrDefault(fieldName, null);
            if (fieldEntity == null)
                throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{fieldName, FieldEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
            return fieldEntity.getBaseType();
        }
    }

    private IndicatorFieldBaseType getDoubleMapFieldBaseType(String fieldName) {
        if (IndicatorDoubleMapEntity.Fields.val.equals(fieldName))
            return IndicatorFieldBaseType.Double;
        else if (IndicatorDoubleMapEntity.Fields.key.equals(fieldName))
            return IndicatorFieldBaseType.Keyword;
        else {
    ...
    private IndicatorFieldBaseType getIntegerMapFieldBaseType(String fieldName) {
        if (IndicatorIntegerMapEntity.Fields.val.equals(fieldName))
            return IndicatorFieldBaseType.Integer;
        else if (IndicatorIntegerMapEntity.Fields.key.equals(fieldName))
            return IndicatorFieldBaseType.Keyword;
        else {
    ...
    public String getGlobalSearchConfig(String key) throws InvalidApplicationException {
        ...
        if (userSetting == null && this.tenantScope.isSet())
            userSetting = this.queryFactory.query(UserSettingsQuery.class).types(UserSettingsType.GlobalSearch).keys(key).entityTypes(UserSettingsEntityType.Tenant).entityIds(this.tenantScope.getTenant()).firstAs(new BaseFieldSet().ensure(UserSettings._key).ensure(UserSettings._value));
        if (userSetting == null)
            userSetting = this.queryFactory.query(UserSettingsQuery.class).types(UserSettingsType.GlobalSearch).keys(key).entityTypes(UserSettingsEntityType.Application).firstAs(new BaseFieldSet().ensure(UserSettings._key).ensure(UserSettings._value));
        if (userSetting == null)
            throw new MyNotFoundException(messageSource.getMessage("General_ItemNotFound", new Object[]{key, UserSettingsEntity.class.getSimpleName()}, LocaleContextHolder.getLocale()));
