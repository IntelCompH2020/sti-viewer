...
public class IndicatorGroupAccessConfigViewBuilder extends BaseBuilder<IndicatorGroupAccessConfigView, IndicatorGroupAccessConfigViewEntity> {
    ...
    public List<IndicatorGroupAccessConfigView> build(FieldSet fields, List<IndicatorGroupAccessConfigViewEntity> data) throws MyApplicationException {
        ...
        Map<UUID, IndicatorGroup> indicatorGroupItemsMap = this.collectIndicatorGroups(indicatorGroupFields, data.stream().map(IndicatorGroupAccessConfigViewEntity::getId).distinct().collect(Collectors.toList()));

        FieldSet filterColumnsFields = fields.extractPrefixed(this.asPrefix(IndicatorGroupAccessConfigView._filterColumns));

        List<IndicatorGroupAccessConfigView> models = new ArrayList<>(100);
        ...
        for (IndicatorGroupAccessConfigViewEntity d : data) {
            ...
            if (!filterColumnsFields.isEmpty() && d.getFilterColumns() != null)
                m.setFilterColumns(this.builderFactory.builder(IndicatorGroupAccessColumnConfigViewBuilder.class).authorize(this.authorize).build(filterColumnsFields, d.getFilterColumns()));
            if (!indicatorGroupFields.isEmpty() && indicatorGroupItemsMap != null && indicatorGroupItemsMap.containsKey(d.getId()))
                m.setGroup(indicatorGroupItemsMap.get(d.getId()));
            models.add(m);
        }
    ...
    private Map<UUID, IndicatorGroup> collectIndicatorGroups(FieldSet fields, List<UUID> ids) throws MyApplicationException {
        ...
        if (!fields.hasField(Indicator._id)) {
            itemMap = itemMap.values().stream().filter(Objects::nonNull).peek(x -> x.setId(null)).collect(Collectors.toMap(IndicatorGroup::getId, Function.identity()));
        }
