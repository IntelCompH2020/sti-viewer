...
public class IndicatorPointBuilder extends BaseBuilder<IndicatorPoint, IndicatorPointEntity> {
    ...
    public IndicatorPointBuilder(
        ...
        super(conventionService, new LoggerService(LoggerFactory.getLogger(IndicatorPointBuilder.class)));
    ...
    public List<IndicatorPoint> build(FieldSet fields, List<IndicatorPointEntity> data) throws MyApplicationException {
        ...
        if (data == null)
            return models;
        for (IndicatorPointEntity d : data) {
            IndicatorPoint m = new IndicatorPoint();
            if (fields.hasField(this.asIndexer(IndicatorPoint._id)))
                m.setId(d.getId());
            if (fields.hasField(this.asIndexer(IndicatorPoint._batchId)))
                m.setBatchId(d.getBatchId());
            if (fields.hasField(this.asIndexer(IndicatorPoint._batchTimestamp)))
                m.setBatchTimestamp(d.getBatchTimestamp());
            if (fields.hasField(this.asIndexer(IndicatorPoint._timestamp)))
                m.setTimestamp(d.getTimestamp());
            if (fields.hasField(this.asIndexer(IndicatorPoint._groupHash)))
                m.setGroupHash(d.getGroupHash());
            if (!groupInfoFields.isEmpty() && d.getGroupInfo() != null)
                m.setGroupInfo(this.builderFactory.builder(DataGroupInfoBuilder.class).authorize(this.authorize).build(groupInfoFields, d.getGroupInfo()));
            if (d.getProperties() != null) {
                Map<String, Object> properties = new HashMap<>();
                for (Map.Entry<String, Object> prop : d.getProperties().entrySet()) {
                    if (fields.hasField(this.asIndexer(prop.getKey())))
                        properties.put(prop.getKey(), prop.getValue());
                }
                if (!properties.isEmpty()) m.setProperties(properties);
            }
            models.add(m);
        }
