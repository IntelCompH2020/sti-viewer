...
public class BrowseDataTreeConfigBuilder extends BaseBuilder<DataTreeConfig, DataTreeConfigEntity> {
    ...
    public BrowseDataTreeConfigBuilder(BuilderFactory builderFactory,
    ...
    public List<DataTreeConfig> build(FieldSet fields, List<DataTreeConfigEntity> data) throws MyApplicationException {
        ...
        List<DataTreeConfig> dataTreeConfigs = new ArrayList<>(100);
        ...
        Map<String, IndicatorGroup> indicatorGroupItemsMap = this.collectIndicatorGroups(indicatorGroupFields, data.stream().map(DataTreeConfigEntity::getIndicatorGroupCode).distinct().collect(Collectors.toList()));

        for (DataTreeConfigEntity d : data) {
            DataTreeConfig m = new DataTreeConfig();

            if (fields.hasField(this.asIndexer(DataTreeConfig._id)))
                m.setId(d.getId());
            if (fields.hasField(this.asIndexer(DataTreeConfig._order)))
                m.setOrder(d.getOrder());
            if (fields.hasField(this.asIndexer(DataTreeConfig._goTo)))
                m.setGoTo(d.getGoTo());
            if (fields.hasField(this.asIndexer(DataTreeConfig._name)))
                m.setName(d.getName());
            if (!levelConfigsFields.isEmpty() && d.getLevelConfigs() != null)
                m.setLevelConfigs(this.builderFactory.builder(BrowseDataTreeLevelConfigBuilder.class).authorize(this.authorize).build(levelConfigsFields, d.getLevelConfigs()));
    ...
    private Map<String, IndicatorGroup> collectIndicatorGroups(FieldSet fields, List<String> codes) throws MyApplicationException {
        ...
        if (!fields.hasField(Indicator._id)) {
            itemMap = itemMap.values().stream().filter(Objects::nonNull).peek(x -> x.setId(null)).collect(Collectors.toMap(i -> i.getId().toString(), Function.identity()));
        }
