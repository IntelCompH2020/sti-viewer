...
public class DataAccessRequestQuery extends QueryBase<DataAccessRequestEntity> {
	...
	protected <X, Y> Predicate applyAuthZ(QueryContext<X, Y> queryContext) {
		...
		if (this.authorize.contains(AuthorizationFlags.Permission) && this.authService.authorize(Permission.BrowseDataAccessRequest)) return null;
		UUID ownerId = null;
	...
	protected String fieldNameOf(FieldResolver item) {
		if (item.match(DataAccessRequest._id)) return DataAccessRequestEntity._id;
		else if (item.match(DataAccessRequest._status)) return DataAccessRequestEntity._status;
		else if (item.match(DataAccessRequest._user, UserEntity._id)) return DataAccessRequestEntity._userId;
		else if (item.prefix(DataAccessRequest._user)) return DataAccessRequestEntity._userId;
		else if (item.prefix(DataAccessRequest._tenant, TenantEntity._id)) return DataAccessRequestEntity._tenantId;
		else if (item.prefix(DataAccessRequest._tenant)) return DataAccessRequestEntity._tenantId;
		else if (item.match(DataAccessRequest._createdAt)) return DataAccessRequestEntity._createdAt;
		else if (item.match(DataAccessRequest._updatedAt)) return DataAccessRequestEntity._updatedAt;
		else if (item.match(DataAccessRequest._config)) return DataAccessRequestEntity._config;
		else if (item.prefix(DataAccessRequest._config)) return DataAccessRequestEntity._config;
		else return null;
	}
